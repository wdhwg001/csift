...
**Args (matches `cli::WhoamiArgs`):** an OPTIONAL positional SELF token (`@trap:<marker>` | `@main`), `--show-path`, and `--format text|json`. The resolved `path` line is ALREADY printed by default whenever the id resolves; `--show-path` only FORCES a `path <not found …>` line in the unresolved case (instead of omitting it). **Self-token:** with NO token (or `@main`) `whoami` is the env path above; `@trap:<marker>` answers "which SUBAGENT am I?" (§6.3a) — it resolves the calling subagent's bare hex + parent uuid **env-INDEPENDENTLY** (reliable for a built-in Task AND a workflow subagent, whose env id is the PARENT), and its JSON carries the full `{session_id, is_subagent, parent_session_id, path}` trio (the env form carries only `{session_id, path}`). `whoami` accepts NO other target (no project path / bare `@<uuid>`) — to inspect a DIFFERENT session use `list`/`agents`.
