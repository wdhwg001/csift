...
### 4.10 The SURVIVAL AXIS: which records the conversation still reaches (v0.12.0)
...
3. **Survival** (§4.10): a per-RECORD verdict from Claude Code's own conversation chain.
...
> **v0.12.2 CHANGE LEDGER (non-breaking; one new leaf and one matching fix -- `csift 0.12.2`).**
...
§4.10 added is ONE extra pass: every full-scan command now BYTE-WALKS each line its own
