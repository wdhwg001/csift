...
| `--full` | — | bool | off | emit each record's FULL text instead of the centered ~400-char excerpt (alias `--no-truncate`). When NOT set and ≥1 excerpt is clipped, a trailing reader-caution prints (text) / `excerpts_truncated:true` (JSON): an excerpt is a match-centered FRAGMENT, not a summary, and can misrepresent the record's full intent — re-fetch via `--full` or `--line`/`--uuid` |
