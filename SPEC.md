...
- **The writers law (v0.11.0, amending "csift only reads").** Through v0.10.5 csift wrote nothing. From v0.11.0 exactly three subcommands write - `send`, `deliver`, `ack` - and they write exactly one place: the csift-owned sidecar directory `<session-sidecar-dir>/csift-channel/` (§1.1). Never a transcript, never the team mailbox, never the cross-session messaging socket, never the session registry, never a settings file. csift never installs a hook (`deliver --recipe` prints the block the user pastes) and never performs an official send (it prints the exact call for the model to make). Every other subcommand is read-only, as before.
...
### 1.1 The csift-channel sidecar (v0.11.0) — the ONLY thing csift writes

The channel (§6.15-§6.17) keeps its whole state in files csift owns, inside the same per-session sidecar directory the elicitation sidecar uses (§6.10) and never inside a transcript:

```
~/.claude/projects/<ENCODED>/<session-uuid>/csift-channel/
  messages/<id>.json      the message SOURCE, written once at send and never rewritten
  outbox.jsonl            the SENDER's log, one line per send
  inbox/<lane>.jsonl      what was enqueued FOR one receiver lane (append-only)
  ledger/<lane>.jsonl     what csift DID about those messages (the intent record)
  armed/<lane>.json       the runtime marker `deliver` rewrites on every event it handles
```

`<lane>` is the TRANSCRIPT-form id (a top-level session uuid, a bare `a<16 hex>` agent id, or a teammate `a<Name>-<16 hex>`) — validated against the crate's existing shape predicates, so every id `agents` prints is a usable lane id and no separator, `..` or drive letter can enter a path component. A message id is exactly 16 lowercase hex characters.

Record shapes, one JSON object per line, no pretty printing:

- `messages/<id>.json`: `{id, ts_utc, from:{kind:"lane"|"external", session, lane, label, cwd}, to:{session, lane, form:"transcript"|"routing", routing_id}, mode, ttl_secs, relation, cross_project, body}`. `relation` ∈ `parent | child | sibling | cross-session | cross-project | external | unknown`; the four peer-ish values are what makes the envelope print its no-authority caution (§6.15).
- `outbox.jsonl`: `{id, ts_utc, to_lane, to_session, mode, verdict, channel, official:{delegated, tool, to_form}}`. It lives in the SENDER's own session directory, so a sender can audit what it sent without reading any receiver's files; an EXTERNAL caller has no session of its own, so its outbox line falls back to the receiver's root. This is also why the body lives in `messages/<id>.json` and not here: the message file is what a delivery reads, the outbox line is what the sender keeps.
- `inbox/<lane>.jsonl`: `{id, enqueued_utc, mode, expires_utc}`. A message is NEVER removed from an inbox; delivery state lives in the ledger, so the two files answer different questions ("what was addressed here" vs "what happened to it").
- `ledger/<lane>.jsonl`: `{id, kind:"emit", event, slot, part, parts, vehicle, ts_utc, hook_session, hook_agent_id, block_count}` per emitted chunk, plus `{id, kind:"held", reason, ts_utc}`, `{id, kind:"expired", ts_utc}`, `{id, kind:"ack", ts_utc}`, `{id, kind:"redelivered", source:"compact"|"resume", ts_utc}` and `{id, kind:"refused", reason, ts_utc}` (the refusal's `id` is optional, because a refusal can predate knowing which message it would have been about).
- `armed/<lane>.json`: `{slots_seen:[k…], last_event, last_ts_utc, hook_session, claude_code_version}`.

**Two write shapes, chosen for concurrency, not style.** Appends (`outbox`, `inbox`, `ledger`) open with `append(true).create(true)` — `O_APPEND` on unix, `FILE_APPEND_DATA` on Windows — and write the line plus its newline in ONE `write_all`, because several hook processes fire concurrently on one lane and a seeking append would interleave halves of two lines. The armed marker is rewritten WHOLE through a temp file plus a rename, so a reader racing the writer sees the old marker or the new one, never a truncated one. Reads are tolerant and never silent: a missing file is the ordinary case (nothing was ever sent to that lane) and yields an empty result; a line that does not parse is COUNTED and reported like every other malformed line in csift.

**INTENT versus FACT.** The ledger records what csift emitted; the proof a chunk landed is a `hook_additional_context` attachment in the receiver lane's own transcript carrying the envelope id. `msg` (§6.16) joins the two, and the fact half alone is exactly `csift search '<id>' @<lane> --additional-context`.

**Slot sequencing is NOT in the sidecar.** It lives under the OS temp dir as `csift-deliver-<ppid>-<event>-<lane>/s<k>.done`: Claude Code runs same-event hooks concurrently and concatenates their output in completion order, so slot 1 wipes and recreates the directory, slot k > 1 waits for `s<k-1>.done` (bounded, 50 ms polls), every slot marks itself, and the last removes the directory. A timeout never blocks a delivery: the slot emits anyway and its chunk carries a one-line warning that the order may be disturbed.

### 1.2 The settings cascade as csift reads it (v0.11.0)

`send` and `whoami --to` must answer "will a delivery hook actually run in that lane?", which means folding the RECEIVER's settings the way Claude Code does. Five named scopes, later winning: `userSettings < projectSettings < localSettings < flagSettings < policySettings`, with a plugin `settings` base below the user scope.

- **Files read.** user `<claude-home>/settings.json`; project `<project-root>/.claude/settings.json`; local `<project-root>/.claude/settings.local.json` AND, when a git root exists strictly above the project root, `<git-root>/.claude/settings.local.json` — Claude Code canonicalises the local scope to the git root only when a uid-ownership probe passes, and that probe leaves no trace, so csift reads both paths, folds the git-root one first (the deeper path wins) and REPORTS both. The policy tier is COMPOSED and first-wins, not merged: `<claude-home>/remote-settings.json` (the server-managed cache) is the whole tier when present, and the managed files that exist but did not compose are reported with that reason rather than looking absent; otherwise `<managed-dir>/managed-settings.json` plus every `<managed-dir>/managed-settings.d/*.json` (dot-prefixed names skipped, sorted, which is the order they win in). `<managed-dir>` is `/Library/Application Support/ClaudeCode` (macOS), `C:\Program Files\ClaudeCode` (Windows), `/etc/claude-code` (elsewhere).
- **Two blocks do not follow "later scope wins".** `env` merges PER KEY (a later scope overrides only the keys it names) and csift keeps the winning scope per key, so a gate verdict can name a file instead of asserting. `hooks` CONCATENATES per event: a project scope ADDS to the user's hooks. Plugin hooks bypass the merge in Claude Code and are unioned at read time (`<claude-home>/plugins/**/hooks/hooks.json` and each plugin's `plugin.json` `hooks` field), folded as one source labelled `plugin:<name>` under a bounded walk whose exhaustion is reported.
- **Three policy switches run after the fold.** Policy `disableAllHooks:true` empties the hook set; policy `allowManagedHooksOnly:true`, or a `disableAllHooks:true` set outside the policy tier, leaves the policy hooks alone. The switch that fired is reported and becomes a `send` risk line, because a configured `csift deliver` hook a switch removed will never run.
- **UNOBSERVABLE, and named as such rather than assumed:** `--settings` (a path or inline JSON, materialised per invocation), `--managed-settings` (a spawning parent's in-memory tier), `--setting-sources` / `--restricted` (can empty the user, project and local scopes), the trust-dialog state (before acceptance only user, flag and policy env applies in full), and MDM policy (a managed plist or a registry tree), which csift never reads.
- **Gate verdicts inherit that honesty.** The teams gate reads the settings `env` key: `enabled via settings env (<scope>)`, or `no settings-level enable; shell env and CLI flags are not observable -> unknown; use evidence: teams directories N, teammate lanes N`. The harbor (cross-session messaging) gate reads the registry row, whose `messagingSocketPath` is the one on-disk consequence of the gate being on AND bound: `registry messagingSocketPath present -> on and bound`, else `... absent -> unknown`. A gate csift cannot see enabled is UNKNOWN with its evidence, never assumed off.
- **The delivery-slot census** (`deliver_slots`) reads the merged hooks for one event and returns the `k` of every `csift deliver --slot k` command entry, sorted and deduplicated (the same k under two matchers is one slot in the chain). It is a whitespace token walk, not a regex: the binary token may be bare or path-prefixed, and a library path never builds a pattern that could fail. Tolerant throughout — an unreadable or malformed settings file becomes a note on its source report, so one broken file never hides the rest of the cascade.
...
(`text` and `tool_result` never co-occur in one user record in real data, so "has a `text` block" is a clean genuine signal once carriers and meta are excluded.) `src/model.rs::is_genuine_user` implements (1)–(4); it **additionally** excludes the machine-synthesized markers of §4.2.1–.3 (interrupt strings, `<local-command-stdout>…`, `<command-name>…`) AND an inbound `<teammate-message>`/`<agent-message>`/`<cross-session-message>` (`is_peer_message`, the GOLD §1 + FINDING-2 fix plus C-30 — a peer-agent message is never the operator; detected ONLY at a SECTION BOUNDARY per FINDING-1, so a tag merely QUOTED mid-prose stays the human) so none of them start a turn AS the human (codepoint-safe via `trim_end`/`ends_with`/exact `==`/`starts_with`, never a byte-offset slice).
...
4. an **inbound peer message** (`is_peer_message_record` — a `<teammate-message>`, `<agent-message>` (FINDING-2) OR `<cross-session-message>` (C-30) from another session) — a real delivered message, so it stays a turn-opener even though `is_genuine_user` excludes it (segmentation/turn-COUNT stays byte-stable where peers already opened turns; only the §5 label changes, to `agent.communication.*`). The `<cross-session-message>` record additionally carries `isMeta:true`, `promptSource:"system"`, `userType:"external"` and a top-level `origin` object of `kind:"peer"`; a `queue-operation` enqueue line with the identical content precedes it and is a harness RIDER, never `user.queued`.
...
```
**HARD-WON (verified across all 51 sessions, 0 counterexamples at the time; RE-MEASURED at CC 2.1.258 for v0.10.1):** a **pending/unanswered single-question** AUQ is **never** flushed to jsonl. Only answered AUQs appear. **The multi-question shape (`input.questions` length >= 2) was on disk while its dialog was open** (measured live on macOS and Windows at 2.1.258; three single-question trials in the same sessions stayed unwritten), so the `status`/`wait` tail reads it as `waiting-hitl` without the sidecar, while the single-question shape still needs the sidecar (section 6.13). The split is a TIMING outcome of the persistence frontier, not a rule keyed on the question count: the binary has no question-count branch on its write path and the frontier is unchanged across builds (v0.10.5, ledger ELI-009), so the two observations are stated as measurements and the gate holding the single-question record is an open question. The answer arrives as a `tool_result` whose `content` is a synthesized string:
```
...
| **`compact_boundary`** (= compaction metrics) | `content:"Conversation compacted"`, `logicalParentUuid`, `compactMetadata:{ trigger:"auto"\|"manual", preTokens, postTokens, durationMs, cumulativeDroppedTokens, preCompactDiscoveredTools[], preservedSegment:{headUuid,anchorUuid,tailUuid}, preservedMessages:{anchorUuid,uuids[],allUuids[]}, messagesSummarized }` - nine keys, the last only on a `/rewind` summarize (claim CMP-002) |
...
Most `system` records carry no §5 label and are skippable noise; the one EXCEPTION is `compact_boundary`, which the classifier labels **`harness.compaction.boundary`** (`Record::classify`, engine-verified). **`search`-surfaced (D7):** `search`'s §7 stage-1 transcript-candidate prefilter — which keeps `"role":"user"`/`"role":"assistant"` lines for the 200 MB-file performance contract — ALSO keeps the rare `compact_boundary` record — but that keep is **`-t`-GATED** (a `needs_compact_boundary` bool, `label_selected(&args.categories, "harness.compaction.boundary")`, derived once and `&&`-gating the extra `memmem`): no-`-t` or a selector reaching `harness.compaction.boundary` keeps it; a `-t user`/`-t agent.*` query pays ZERO (the `memmem` is never reached). Behind that gate the keep is a **CONJUNCTION** (C-40): the rare literal `compact_boundary` AND the key-only bytes `"subtype"`, tested in that order, because the literal alone is not the discriminator — a payload can carry the word in prose. Measured over every top-level transcript of one corpus (76 files, 2,661,636,749 bytes, lines split on `\n` only): 252 of 252 true boundaries carry the key and are admitted; 226 of 226 lines carrying the literal only in a payload, which no other flagless keep reaches (189 `attachment` + 37 `queue-operation`, 33,287,888 bytes on 5 files), carry it in none and are refused. A refused line is still lifted to a structural spine row by the non-candidate arm, so the survival walk reads through it, and an explicit address still fetches it whole. The role keeps ALWAYS run (conservative superset); when the boundary `memmem` runs it only touches lines that already failed both role checks, and boundaries are rare — so the perf contract holds. So `search -t harness.compaction.boundary` surfaces it: for a message-less system record `record_raw_text` renders the boundary's top-level `content` plus a readable `compactMetadata` excerpt (`[compaction boundary: trigger=… preTokens=… postTokens=… durationMs=…]`) as the match + excerpt, so compaction points can be enumerated and inspected; `verbatim` still also renders the boundary as its own banner. The compaction SUMMARY (item 1 above) is a `type:"user"` record, so it classifies **`harness.compaction.summary`** AND is surfaced by `search`. All other `system` subtypes still parse cleanly, carry no label, and must never crash time logic.
...
### 4.10 `/clear`: a new session in the same process, joined by the cost ledger (v0.12.2)

`/clear` (aliases `reset`, `new`; optional `[name]`) ends a session and starts another inside the SAME process, and it writes no lineage anywhere. One generator does it, in a fixed order: the SessionEnd hook with reason `clear` on the OLD id; the conversation state emptied; the cost-ledger SAVER; the session id regenerated with `crypto.randomUUID()`; the SessionStart hook with source `clear` on the NEW id. The transcript file handle is dropped, so the next append opens `<new>.jsonl` in the same project directory; the registry row keeps its pid and swaps `sessionId` in place; `$CLAUDE_CODE_SESSION_ID` reports the new id. An SDK consumer's `conversation_reset` frame carries a separately minted uuid that is NOT the session id.

**No field names the predecessor.** The old id lives only as an in-memory `parentId` for the rewind UI. Censused over one projects root, `"parentId"`, `"clearedFrom"` and `"previousSessionId"` occur on zero lines of 9,521 transcripts; a FORK, by contrast, writes `fork-context-ref.parentSessionId` (33 lines there). So any join is an INFERENCE and must say so.

**The instrument is the cost ledger's checkpoint.** The saver runs BEFORE the id swap, so the `cost-state` line it writes carries the OLD session id and lands in the OLD file, and `startTime + totalDuration` on that line is the instant the old session closed. The NEW file opens with the re-stamped session-state lines, then an isMeta `<local-command-caveat>` user record, then the `/clear` slash wrapper itself, then a `system`/`local_command` record with empty stdout. Measured on a live clear the wrapper sits 164 ms before the close instant (the caveat +3 ms, the `local_command` +2 ms) and the nearest competing checkpoint in the same directory was 43,968 ms away. `startTime` is per SESSION and is reset at the clear, so equality of `startTime` across files is not a join; the SUM is. `list` reads exactly that: `minted_by: "clear"` when the first non-isMeta user record is the wrapper, `cleared_from` when a sibling's checkpoint closes within 2000 ms of the wrapper instant, the nearest LINE winning and a tie between two FILES reported and joined to neither (§6.1).

**A checkpoint is repeatable, so it is never a close marker.** The `cost-state` literal has 7 occurrences in the shipped binary (an interned name, the schema, one builder, two policy tables, two readers) and the one writer has five entry points: the registered saver (the clear, a background handover, an in-app resume), a background spawn, a resume or in-app fork, the shutdown-gated teardown, and the exit re-stamp provider - an exit writes TWO lines with the session-metadata re-append between them. Corpus: 14 of 9,519 transcripts carry one, 44 lines, 35 of them not last. The schema is twelve keys with no `uuid`, no `timestamp` and no `message`, so the line is not a chain node and its only address is its physical line; the loader admits only `user | assistant | attachment | system` into the message array, so its values never reach the model except through the restored ledger under `--max-budget-usd`. `status`/`wait` report one at the TAIL as evidence, never as a verdict (§6.12).

**What the clear resets.** In memory: the message array, readFileState, nested memory paths, session env vars, the memory selector, EVERY plan slug, file history, the cost counters, the prompt-assembly sections (system prompt rebuilt, CLAUDE.md and memory files re-read), skills-sent state, slash-command caches, invoked skills, the WebFetch / ToolSearch / agent-definition caches, the metadata cache, and the shell cwd. NOT reset: MCP connections (only the ide server's cache) and the permission cache. Background tasks survive unless a foreground task is still in its turn. No PreCompact or PostCompact hook fires and no summary or boundary is written - a clear is not a compaction. The plan corollary: `forgetPlanSlug` runs on the outgoing id, so a clear-minted transcript has NO plan binding until plan mode is entered again, and `csift plan` reports none rather than inheriting one.

**The task store keeps the startup id.** The list id is `CLAUDE_CODE_TASK_LIST_ID` else the team context's `teamName` else an inherited name else the leader team name else the session id, under `<claude-home>/tasks/<that id>`. With the agent-teams path active the implicit team `session-<first 8 of the startup id>` is minted once at startup, its `config.json` stamps `createdAt` at that instant and a `leadSessionId` written once, and the initializer renames `tasks/<startup uuid>` onto that name. A clear, a resume and a fork never touch it, so the store outlives the session id that named it (§6.12 for the candidates `status` tries).

### 4.11 The SURVIVAL AXIS: which records the conversation still reaches (v0.12.0)

**A transcript is not the conversation.** Claude Code reconstructs a session by streaming every `user`/`assistant`/`attachment`/`system` line with a string `uuid` into a map where the LAST occurrence of a uuid wins, choosing ONE leaf, and walking `parentUuid` from that leaf to the head. It reads `parentUuid` and nothing else, so a record with none ends the walk, and a `compact_boundary` (its own `parentUuid` null) is exactly such a record; `logicalParentUuid` is never read on that path. Everything the walk reaches is the conversation, and everything else is on disk without being in it. `model::Chain` (src/model/chain.rs) mirrors that rule hop for hop and is the single production answer to "is this record still in the conversation?", and it replaced three earlier heuristics that approximated it (the same-parent draft rule, the replay-copy collapse and the boundary's implicit cut), none of which could see a turn the operator rewound past. Claims MISC-034 (the map and its admission gate), MISC-035 (the four leaf gates), MISC-036 (the walk), MISC-037 (the preserved-messages relink), MISC-038 (the two membership passes), MISC-006 and REC-033 (what the DAG shape can and cannot say), MISC-043 (the dangling boundary).

**Three values, `Survival::{Live, PreCut, Abandoned{root}}`.**
- **Live**: on the chain, or widened onto it by the two membership passes that run after the walk: the same-`message.id` assistant siblings of a chain assistant (with the `tool_result` carriers parented to them), and every non-conversation descendant of the leaf.
- **PreCut**: physically above a compaction cut, or inside a region csift's own walk could not resolve. Claude Code drops this history on a resume; csift keeps reading it and EVERY selector still reaches it, flagged and never hidden, because what the model saw AT THE TIME is the question an archive answers. See the blind region below.
- **Abandoned{root}**: reached by neither, with `root` naming the line where the branch left the conversation.

**The leaf choice, and why it is disclosed.** The leaf is the `leafUuid` of the newest `last-prompt` line the file carries, subject to four gates: a later `compact_boundary` WIPES the accumulated set; an `explicit:true` line with a null leaf clears it; a recorded leaf that is a proper ANCESTOR of the file-order-last record yields to that record; and a leaf naming no record in this file falls back to the tail, never to the next-newest `leafUuid`. A `last-prompt` line carries no `uuid` of its own (claim REC-103), so it is a session-metadata snapshot and not a conversation record. Which gate fired rides every surface as `leaf_source`, because the leaf decides the chain and the chain decides the answer.

**Drafts and rewinds are the SAME shape with one discriminator, and it is a DESCENDANT.** An abandoned turn OPENER is a `user.unsent` draft when no assistant record hangs below it (a prompt recalled with esc and re-typed) and a `user.rewound` turn when one does (it was sent, it drew a reply, and the conversation later threaded around it). Measured over the corpus's 910 shared-parent opener pairs: 858 earlier members with no assistant descendant, 52 with one, and 0 of the 910 on the final chain (claim TURN-032). Both labels are RETROACTIVE and neither can be assigned at write time, because the record carries no field that marks it (claims TURN-026, MISC-041) and the deciding sibling or descendant does not exist yet.

**A replay copy is the EARLIER line.** A compaction re-anchor re-appends a contiguous block of already-written records with their uuids preserved. The loader's map keeps the LAST line carrying a uuid, so that line is the survivor for opener treatment and for addressing by uuid, and the earlier line renders `[replay copy of L<n>]`. Both stay searchable and counted, the same double-count law §4.7's clone states, one file down instead of across files.

**The blind region, and the FLOOR that protects it.** csift's walk ends where Claude Code's ends. A `compact_boundary` carries a null `parentUuid` (260 of 260 boundary records over every transcript under one projects root), and the walk's null-parent arm breaks before any other field is read, so the boundary terminates the walk and everything physically above it is a region csift did not resolve. That terminating record is a FLOOR: an off-chain record above it reads `PreCut` and is NEVER called abandoned, and inside that region the measured same-parent opener rule still marks a draft, so draft detection never depends on the chain reaching that far. What csift adds above a cut is not a crossing but a refusal to discard: the loader drops that history, csift keeps every record the walk did not reach, flagged and reachable by every selector. The `logicalParentUuid` arm in the walk is a GUARD for one shape only, a boundary whose own `parentUuid` is non-null and resolves to no record, and the corpus holds zero specimens of it. Renaming the field on five real compacted transcripts of 9 to 21 MB leaves the per-hit `survival` census, the `stats` chain totals and the search-summary disclosures byte-identical, which is the measurement that says the guard is not the path. The blind region is not a partial record set either: no admitted record's `parentUuid` names a progress or unparsed line (12 transcripts probed, empty census). Claim MISC-043 records what the guard would meet if it ever fired: 52 of 255 `compact_boundary` records, across 14 of the 21 compacted transcripts among 77 top-level ones, name a uuid that is on NO line of their own file, the pre-compaction records having been deleted from disk by the pre-boundary deletion path.

**Two fail-open guards on csift's own side.** The §7d prefilter keeps only role-bearing lines, and a chain built from those alone resolves nothing (1 of 30,613 conversation records reachable on one real session), so every surface splices back the STRUCTURAL row of each line its prefilter drops through `parse::spine_record`: `type`, `uuid`, `parentUuid`, `timestamp`, `isSidechain` and the boundary fields, lifted by a depth-1 key walk into a narrow `parse::SpineRow` on its own output stream (v0.12.1; through 0.12.0 it was an otherwise empty `Record` sharing the surface's record vector, §7g), never the payload. A line the prefilter ADMITTED but the label gates leave unsearchable takes the same road from the other side: it is MOVED onto that spine stream as the identical row (`parse::spine_from_record`) instead of being removed, because deleting it would punch a hole in a chain that threads through attachment records.

Where the axis lands per command is §6.4.1 (live turn numbering and the disk / conversation split); what it does to a bare role selector is §5.6.

---

## 5. Label taxonomy — `role.class.sub` (`search -t/--category`, repeatable)

The `-t` axis is a 3-role, **37-leaf** dotted taxonomy (`model::Class`; `Class::ALL` is the single source of truth, pinned by a hand-kept oracle table whose length must equal it). `Record::classify(ctx) -> Vec<Class>` is the engine: **multi-label** (one physical record can carry >1 leaf), pure, tolerant (an unmodeled record → empty `Vec`, never a crash). A selector matches a record iff some selector path is a **dot-SEGMENT prefix** of some label path (so `-t agent` covers the whole role, `-t agent.tool` covers use+result), and the rendered/JSON label is always the full dotted leaf path.
...
├─ user.unsent             a SUPERSEDED turn-opener draft (v0.9.2): sent, esc-recalled into the input box, edited, re-sent — the original stays on disk sharing the resend's parentUuid. Assigned at the SCAN layer (the set needs the LATER sibling; C-18's `superseded_draft_indices` is the one rule), SINGLE label (user.message stays pure), outside turn numbering (header `<tok>·draft`, JSON null turn_index), suppressed under a `--turn` window, always address-reachable. 99% never drew a reply (measured n=317). A rendered draft carries the C-27 DIFF against the message that replaced it (`differs from the sent message in N chars (P% of the final)`, or `identical to the sent message`; JSON `superseding_line`/`superseding_uuid`/`diff_chars`/`diff_pct`/`diff_exact`), and the label itself is RETROACTIVE - the discriminator is the later sibling, so nothing can assign it at prompt-submit time (v0.11.1 ledger ④, claim TURN-026). LIMITS: an abandoned recall (no resend) is structurally undetectable; a queued text edited before dispatch never becomes a user record (its `queue-operation` line is `user.queued`, v0.10.0).
├─ user.rewound            a turn-opener the conversation was REWOUND past (v0.12.0): it was sent AND it drew a reply, and Claude Code's own `parentUuid` chain now threads around it, so neither it nor anything under it is in the surviving conversation. The discriminator against `user.unsent` is exactly that reply - a recalled draft has no assistant descendant, a rewound turn does (measured over the shared-parentUuid opener pairs of a real corpus: 858 without, 52 with, 0 on the chain). Assigned at the SCAN layer for the same reason `user.unsent` is - the fact lives in the DAG around the record, not in the record - so it is equally RETROACTIVE. Outside turn numbering (header `<tok>·rewound turn`, JSON null turn_index), suppressed under a `--turn` window, always address-reachable, and its C-27 line leads `rewound: the conversation continued from L<n> instead`. NOT LLM-visible: a bare `-t user` skips it, a glob or the full path reaches it. Every OTHER record of the rewound branch keeps its ordinary leaf and carries an `[abandoned]` / `[rewound]` label-zone marker.
...
└─ agent.communication                     (EVERY comm hit renders  from ⇨ to)
   ...
   └─ channel              (v0.11.0) a csift-channel DELIVERY: a `type:"attachment"` record whose payload is `hook_additional_context` and whose FIRST content string opens with `[csift-channel v1 ` (§6.17). It carries `harness.meta.hook` too and LEADS it (richest-view law): the record is hook machinery on disk and a message addressed at this lane in meaning. The one attachment leaf a DEFAULT scan reaches — the candidate needle `csift-channel v1` is a bare value substring with no JSON-escaped character (R13-safe) and is gated on the label selector alone, so `-t user` / `-t harness` short-circuit before the memmem and pay nothing. Rendered VERBATIM (the header line plus the body chunk, a byte substring of the decoded content), so the §7d prefilter and the §7f whole-file gate hold with no synthesized-marker machinery; direction comes from the header's `from=` (`<sender lane> ⇨ self`), and a CONTINUATION chunk, whose header carries only `id=` and `part=`, reports no direction rather than a guessed one.
...
├─ harness.schedule.wakeup                       fired ScheduleWakeup/autonomous-loop timer tick. Its markers - the `<<autonomous-loop-dynamic>>` sentinel, the `# Autonomous loop check` header and the `You're being invoked on a timer` body sentence - are matched at CONTENT START only (v0.12.2), after the same `trim_start` the resume-prompt arm uses. The resolver builds the WHOLE delivered prompt and the fire writes it as its own record, so a real tick carries its marker at offset 0; a marker deeper in a body belongs to a record QUOTING the prompt (a skill's instruction text that embeds the preamble, a peer relay, a compaction summary recapping one), and matching anywhere read each of those as the tick and stripped the leaf it had earned. `harness.meta.loop` anchors the same way.
├─ harness.schedule.fire                         (v0.12.2) the PROMPT a scheduled task FIRES: a cron entry or a `ScheduleWakeup` timer comes due and Claude Code submits the armed text as its own `type:"user"` record - `isMeta:true`, `promptSource:"system"`, `userType:"external"` - parented to the `system`/`scheduled_task_fire` record written one line earlier, whose `content` names the instant in a trailing parenthesis (`Running scheduled task (<when>)` for a cron task, `Claude resuming /loop wakeup (<when>)` for a wakeup; one formatter, two wordings, so csift reads the parenthesised TAIL rather than either sentence). The armed text reaches the record VERBATIM - the fire path applies exactly ONE rewrite, the autonomous-loop sentinel resolution, and every other prompt falls through it unchanged - so there is NO marker to match and this leaf keys on the record's own FIELDS. Its arm therefore sits LAST among the harness marker arms: a fired prompt may legitimately open with any of them, and a tick that really carries one keeps `harness.schedule.wakeup` / `harness.meta.loop` by arm order. `promptSource:"system"` is not the scheduler's own stamp (the submit path puts it on EVERY `isMeta` submission, and Claude Code's own reader calls the `isMeta` + `promptSource` pair a system-injected turn prompt while testing `isMeta` + NO `promptSource` for a human entry), so an inbound peer message shares it exactly and TWO guards refuse one: the relay FRAMING through the same `is_peer_message` predicate `user.queued` uses, and the `origin` OBJECT, which the harness stamps on everything it did not submit itself and reads back as the first test of its own foreign-input veto - the framing guard alone misses a TAGLESS delivery. The `queue-operation` line carrying the identical text one line earlier stays UNLABELED (a harness rider, the same content-shape law). DELIVERED (the model answers it as an ordinary turn) and never a turn OPENER, like every `isMeta` record. The INSTANT is a cross-record fact, so it rides the LABEL ZONE (`[scheduled fire <when>]`) and JSON `scheduled_at`, display-only like `[summarize <dir>]` and for the same reason - matchable text drawn from another record would need a synthesized-marker registration it does not earn. It is joined by `parentUuid` through a per-file `ScheduleFireIndex`, built from the fire lines one more `&&`-gated candidate keep admits (a C-40 conjunction of the rare literal `scheduled_task_fire` AND the key-only `"subtype"` bytes; measured over the 80 top-level transcripts of one corpus, all 465 records with top-level `type:"system"` and `subtype:"scheduled_task_fire"` carry the key, and of the lines that carry the literal WITHOUT being such a record not one does - 216 of them at the instant of the measurement, 72 reached by no role keep and so refused by this conjunction alone. That second population is not a fixed number and must not be read as one: 211 of the 216 sat in the transcript of the session doing the measuring, because a session writing ABOUT the literal accumulates it in prose. Which is precisely the hazard - the discriminator is the KEY, whose presence is a property of the producer, never the literal's rarity) and DEMOTED back to spine rows once read, so a flagless scan gains no `harness.meta.system` hit from the keep. A transcript holding no fire record reports a null instant, never a guessed one.
├─ harness.resume.prompt                         (v0.12.0, renamed from `harness.schedule.continuation` — same predicate, same records) the repair PROMPT the resume LOADER appends when the transcript it loaded ends on a dangling `type:"user"` record: an `isMeta` user record whose text opens with `Continue from where you left off.` The producer is the resume post-processor, never the scheduler, which is why the leaf moved families. The predicate is `isMeta` AND `trim_start().starts_with` — not equality — because the sentence is the DEFAULT of an env-overridable getter and Claude Code sets two longer variants for its own respawns; Claude Code's own recogniser compares exactly against the value ITS environment resolved, which a transcript cannot show. **The `isMeta` half is what protects a person**: it is the authorship flag the injecting call site stamps, Claude Code's recogniser tests it too, and a human message that merely OPENS with the sentence carries no such flag — so it stays `user.message` and opens its turn as always. Both exclusions this leaf claims — never a turn opener, never the operator — hold THROUGH that gate, which is also what `is_genuine_user` reads. (csift does not apply the recogniser's third test, `promptSource` absent, so its set is a superset by exactly the records carrying that field; none exists in the corpus.) DELIVERED: the model receives it. The retired selector spelling is a hard parse error naming this path.
├─ harness.resume.placeholder                    (v0.12.0) the stand-in reply the same loader splices in right after a resumed transcript's trailing user record, so the loaded conversation does not end unanswered: a `type:"assistant"` record with `message.model` `<synthetic>`, no `isApiErrorMessage`, and a single-text content of exactly `No response requested.` — a 1:1 port of Claude Code's own recogniser. Fabricated with no model call, so it is NOT `agent.message`. ONE writer produces it and its gate is only "the tail is a user record", while the prompt's gate is the tail classifier's interrupted-turn verdict — so the same record also appears ALONE after an interrupt marker, a `<local-command-stdout>` echo or a slash wrapper. That difference is a FACT on the hit, not a second leaf: `resume_paired` (JSON) / `[paired]` `[unpaired]` (label zone), computed from the record's `parentUuid` against the file's repair-prompt uuids. DELIVERED: the request assembler's `<synthetic>` arm also requires `isApiErrorMessage`, which the loader does not set.
...
| inbound `<teammate-message>`/`<agent-message>`/`<cross-session-message>` prose | `agent.communication.inbox` | — | from ⇨ self | `…inbox` |
...
| csift-channel delivery attachment (v0.11.0) | `agent.communication.channel` | `harness.meta.hook` | from ⇨ self | `…channel` |
...
- `<cross-session-message>` (C-30): FROM = the `from-name` attribute when present, else `from` — `from` is a transport address (`uds:<socket path>`, a bridge id), `from-name` is the sending session's display name, so the name is the readable side. On a record carrying exactly ONE peer section the structured `origin.name` wins over the attribute; they are the same value (Claude Code fills `origin` by re-parsing the tag), and a BATCHED record reads each section's own attribute instead.
...
- csift-channel delivery (v0.11.0): FROM = the envelope header's `from=` value (a lane id, or `external:<label>`); owner = TO. A continuation chunk names no sender and therefore renders no direction.
...
### 5.5 Record-level delivery (v0.11.1): what a bare ROLE selector means

A bare ROLE selector claims to show the conversation the model received. That is a
per-RECORD fact, not a per-leaf one, and Claude Code decides it with ONE drop predicate
inside the assembler that turns a transcript into an API request (claim CLS-026). The
predicate disagrees with the leaf table of §5.1 in exactly THREE cases, and
`Record::delivery_override` mirrors those three and nothing else:

| record shape | leaf default | the harness | csift |
|---|---|---|---|
| `type:"system"` + `subtype:"local_command"` | invisible (`harness.meta.system`) | RE-MINTS it as a user message and sends it | `Some(true)`: a bare `-t harness` surfaces it |
| `type:"user"`/`"assistant"` + `isVirtual:true` | visible | drops it (display-only by construction) | `Some(false)`: a bare role hides it |
| `type:"assistant"` + `isApiErrorMessage:true` + `message.model=="<synthetic>"` | visible | drops it (claim CLS-027) | `Some(false)`: a bare role hides it |

Every other record carries no override, so its leaf default stands. The predicate's two
remaining arms need none: it KEEPS an `api_system` record and drops a `progress` record,
and csift models no leaf for either type, so neither is selectable at all.

Only the BARE-ROLE form consults the override. A glob (`-t 'agent.*'`), an intermediate
prefix (`-t harness.meta`) and an exact leaf path keep their full sets, so every record
stays reachable by naming it; `-T` speaks the same rule on the exclude side. The label
SET is untouched (`labels[]` is classification, never the delivery verdict); the verdict
rides beside it as the JSON hit field `delivered` (the override, else the leaf default)
and, on an undelivered record under an explicit selector, as the display-only label-zone
marker `[not delivered]`.

Candidate admission follows: `harness.meta.system` was reachable only under an explicit
selector, so a bare `-t harness` could not have seen the delivered record at all. The
key-only `"subtype"` needle is therefore also enabled under a bare `harness` role
(`SearchArgs::scans_system_lines`), and the per-record override drops the rest of what
that admission parsed. The §7f whole-file gate's synthesized-marker set reads the SAME
predicate: a narrower set would prune a file whose only hit lies in the fabricated
`[<subtype> <level>]` head, which is not a raw byte substring of the line. Every other
gated leaf is unchanged, and `gated_leaves_unreached` now reports what the scan actually
parsed.

Measured on the reference corpus, 72 top-level transcripts, one observation per record:
16 `system`/`local_command` records (12 carrying a `<local-command-stdout>` body, 4 being
command echoes) appear under a bare `-t harness` where 0.11.0 showed none, and
`agent.message` drops by exactly 118, the whole population of records carrying
`isApiErrorMessage` (118 of 118 also carry the `<synthetic>` model).

### 5.6 Record-level survival (v0.12.0): the THIRD exclusion axis

A bare ROLE selector narrows on three axes, applied in this order, and only the first is
per-leaf:

1. **Leaf visibility** (§5.1): nine leaves sit outside a bare role by default
   (`Class::llm_visible()` is false for exactly these), and they split by WHY. SEVEN are
   non-message line types and ONE instrument covers all seven: none of them carries a
   `message{}` field, while every conversation record does. The other TWO, `user.unsent`
   and `user.rewound`, are ordinary user records whose line shape excludes nothing; what
   excludes them is axis 3 below, which is why this law needs three axes and not one.
2. **Delivery** (§5.5): a per-RECORD mirror of Claude Code's own request-assembler drop
   predicate, disagreeing with the leaf table in exactly three cases.
3. **Survival** (§4.11): a per-RECORD verdict from Claude Code's own conversation chain.
   A record the chain no longer reaches is not in the conversation the model received, so
   a bare role skips it.

The three are independent and the second and third are the harness's own rules, not
csift's readings of them: a record can be a visible leaf, delivered when it was written,
and abandoned today. Every OTHER selector form is untouched by all three: a glob
(`-t 'user.*'`), an intermediate prefix (`-t agent.tool`) and a full leaf path reach every
record whatever the three axes say, and an excluded record then renders its reason in the
label zone (`[not delivered]` for axis 2; `[abandoned]`, `[rewound]` or `[pre-cut]` for
axis 3). A `PreCut` record is NOT excluded by axis 3: csift declines to narrow on a
verdict it could not resolve.

JSON carries the verdicts beside the labels, never inside them: `delivered` for axis 2,
`survival` plus `abandoned_root_line` and `replay_copy_of` for axis 3. `labels[]` stays
classification.
...
> **v0.12.3 CHANGE LEDGER (non-breaking; one added JSON field; `verbatim`'s fold accounting; ported-checker fidelity; the session-lineage leftovers -- `csift 0.12.3`).**
...
> ② **The fold marker names what it folded.** A collapsed agent-message run rendered `[X agent message(s), Y tool call(s)[, Z failed]]` -- counts and a line range, with nothing about the prose it stood for, so a folded finding and a folded "let me look into this" read identically. The marker is now `△ L{first}–L{last}  [X agent message(s) collapsed, N chars, Y tool call(s)[, Z failed]]`, where N is the summed `full_chars` of the collapsed bodies, and every collapsed message of at least `COLLAPSED_PREVIEW_MIN_CHARS` (210) is followed by an indented preview line `    L{n}  {first COLLAPSED_PREVIEW_CHARS=60 chars}… (+K chars)` through `text::truncate_excerpt`, so the excerpt states its own remainder. 210 sits ABOVE the pure-declaration band (`declaration_max_chars` 200 at the default profile, 240 at `light`), so a preview means "this was not a declaration"; the band a preview can occupy at all is 210 up to `rich_min_chars` (280 default, 360 at `light`), because a message at or above the rich threshold is KEPT rather than folded. `turns::agent_placeholder_lines` is the ONE list the renderer emits and `agent_placeholder_cost` charges, so the summed-cost == summed-emitted invariant covers the previews too. JSON `collapsed_agents` rows gain `collapsed_chars` and `collapsed_previews[]` (`{line, excerpt}` each). Cost on the installed hook's path (`--slices 4 --window 9000`), measured over the two largest top-level transcripts of one corpus: 2 and 3 fold markers in the whole four-slice document, 0 preview lines, and +41 / +61 chars from the `N chars` clause on a ~24,700-char document. At an unbounded budget over a WHOLE session the preview band is reached by 42 of 1,224 and 212 of 1,759 folded messages.
> ③ **The `L lines elided` note counts the newlines the CUT removed.** It used to be the whole body's newline count, which made it independent of how much was actually deleted: two different cuts of one message printed the same figure, and a 17-char cut could claim 95 lines. Measured over the two largest top-level transcripts of one corpus, 18 of 2,480 truncated units reported MORE elided lines than elided characters -- arithmetically impossible, and the cheapest proof the figure was not describing the cut. A `TurnUnit` now carries `newline_positions`: the char offsets, in the one-line rendered text's own coordinates, at which the original body's newlines landed (a newline is not a character of the collapsed form, so its address is the space that replaced its whitespace run; newlines collapsed together share an offset and are each counted). `turns::elided_newlines` counts the offsets STRICTLY INSIDE the removed span `[cut_from, cut_to)`, so an uncut body reads 0, a cut falling between two newlines reads 0, and a wider cut of the same message reads more. The positions are recorded only when the rendered text IS the normalization of the record's raw body; a FABRICATED body (an AskUserQuestion scaffold, an automation attribution label, a peer-message preview) reports no lines, because the raw record's newlines sit nowhere in a string csift composed -- the old figure was measured against a different string. `model::normalize_line_with_newlines` shares the ONE collapse walk with `normalize_line`, so the string and the offsets cannot drift.
> ④ **A same-prefix RE-SEND folds to one line** (claim MSG-095; the shape rests on CLS-016, the `Stop hook feedback:` isMeta record, and MSG-017, the exit-2 vehicle that produces it). When one turn carries two assistant messages and the LATER one's body CONTAINS the earlier one's whole body -- a re-send with an addendum -- the earlier renders as `△ L{n}  [superseded by the same-prefix re-send at L{m}, N chars]` (N = the suppressed body's own char count) instead of a second copy of the prose, and the survivor prints in full. `turns::mark_resends` is a second stage over the keep/collapse lane: only KEPT entries participate (a message already inside a placeholder is suppressed anyway, and the survivor must itself be rendered, or the fold would hide both bodies), the comparison key is the summary-dedup normalization (`normalize_line` + lowercase, the `DEDUP_PREFIX`-wide head grouping the candidates and the FULL text deciding containment), and a body under `DEDUP_PREFIX` chars never qualifies -- the fingerprint is only the strict discriminator its own doc claims at full width, and below it the marker costs more than the body. JSON emits the superseded unit's row IN FULL with `superseded_by_line`; the text fold loses nothing a machine reader needs. **Why containment and not prefix equality:** censused over every top-level transcript of one corpus (80 files, 6,468 turns, 35,052 assistant messages), 53 same-turn pairs shared their first 80 normalized chars, of which 1 was a true duplicate, 25 diverged before char 120, and 22 had a SHORTER later member -- a fold on the 80-char match alone would have suppressed prose the survivor does not carry. Under containment the same corpus yields 2 folds, both exact duplicates. **Why the name says same-prefix and not Stop-hook:** of that corpus's 36 `^Stop hook feedback` records, 30 have their nearest assistant neighbours in the same turn and 0 of those pairs share 20 or more normalized chars (29 share none), and no same-prefix pair has a feedback record between its members -- so the rule is named for what it matches.
> ⑤ **The docs say WHICH path the per-role caps apply to** (documentation only; `ASST_CAP` is unchanged). The 600/900 body caps are the DIRECT path's -- a plain run, `--out`, and the legacy budget-driven `--slice i` alone -- while `--slices N`, the fixed-fleet hook-injection path, drops them for a window cap and holds its slice count by DISCARDING the oldest turns that do not fit. A reader who met a 900-char assistant body on one path had no way to know the other path does not cap at all, and the `--slices` flag doc was the only place that said so. §6.8 now states it beside the ellipsis rule, with the measured cost of the direct cap: over the last 40 turns of the two largest top-level transcripts of one corpus, the 900-char cap keeps 32,404 of 59,965 assistant chars (27 of 42 units cut) and 67,846 of 117,878 (48 of 101 cut); at 1,800 those become 50,657 (12 cut) and 95,262 (18 cut), so raising it would keep 18,253 and 27,416 more chars. The number is recorded for the owner's decision, not acted on.
> ⑥ **The generation-3 blanking mask reads a backslash with the harness's own rule** (claim BASH-041; AGENTS §3.9 carries the generation table). `out` masks its input twice before the clause walk starts - once blanking every quoted run and comment, for the function-definition scan, once keeping them, for the `set --` scan - and those two answers decide the POSITIONAL allowance, the flag the `$1`/`$@`/`$*`/`$!` target form is gated on. csift's mask treated every backslash outside a single quote as an escape and emitted one character per character. The harness decides it with the same `RNe` the shield reads - never an escape inside a single quote, always one outside quotes, and inside a double quote only before `"`, `\`, `$` or a backtick - and blanks an in-quote escape as a PAIR, so an escaped quote does not close the run that hides it. The old reading ended a double-quoted run at the first escaped quote and flipped the quote state for the whole remainder of the command, which reads a function definition written INSIDE a quoted string as a real one and blanks a definition that genuinely FOLLOWS the string. Both directions are real: over 165,314 distinct (Bash command, version) pairs of one corpus the two readings disagree on 4,097 masked strings and on 165 function-scan answers, 145 of them where the pair rule hides a definition the character rule revealed and 20 the other way. NO corpus lane changes verdict and no output byte moves, for two reasons that are properties of the corpus rather than of the rule: the generations before the rewrite never call a mask at all, and the three differing commands recorded at or past the rewrite floor carry no `$`, which the classifier's entry guard wants before the mask is read. The rule is observable exactly where the escaped character IS a quote, because a blanked pair is two blanks either way - what moves is the quote state, and with it which later text is blanked and whether a later `#` opens a comment.
> ⑦ **`list` names the child a BACKGROUND HANDOFF continued the session in** (claim REC-106). A session handed to a background child - the left-arrow gesture, or the background-fork command - leaves one line in the PARENT transcript: `{type:"continued-in", timestamp, sessionId, continuedInSessionId}`, four keys and no `uuid`, so it is not a chain node and its only address is its jsonl line. This is the THIRD lineage link on a `list` row and the only one that is not an inference: `clone_of` is read off a boundary shape and `cleared_from` off a checkpoint sum, while here Claude Code writes the child id itself. Text adds one `handoff` row naming the child's first-8 token; JSON adds `continued_in` (the full id, null when absent). Two facts bound the reading. An in-place `/fork` passes `keepParent` and writes no such line, so absence never means "not forked"; and the append also happens on the failed-spawn path once a rescue row is queued, so presence does not prove the child ran - Claude Code's own reader opens the named child and demands conversation content in it before hiding the parent from the resume picker. WINDOW LIMIT, stated because `list` is a head/tail reader and not a census: the line is appended after the last conversation record, so the backward tail walk normally reaches it before the anchors that stop the walk, but a later `--resume` of the parent appends turns below it and can push it outside both windows, where `continued_in` reads null. The whole-file answer is `stats`' line-type census, which already counts `continued-in` as one of its `types` keys. Cost: the line carries no `role`, so the `list` prefilter gains one `||`-arm memmem on the quoted KEY - the R13 key-only form, which survives a reserialize and which a prose mention of the type value cannot trip.
> ⑧ **`list` reports the BACKGROUND-LANE stamp `sessionKind`, and `--lineage` gives its SPAN** (claim REC-107). Every record a background lane writes carries a top-level `sessionKind:"bg"`; a foreground lane's records carry no such key at all, because ONE stamp builds the value from ONE environment variable whose value is undefined there and `JSON.stringify` drops an undefined property. The key is therefore a per-PROCESS fact, never a per-record one, and it stops appearing in the SAME transcript once a foreground process appends to it - which is exactly what a resume of a backgrounded session does. So `session_kind` is an ARRAY (text row `lane`), EMPTY for an ordinary session rather than a sentinel value, read from the head and tail windows - which is also where Claude Code's own session summary reads it, off the FIRST record of the file. The SPAN cannot come from a window, so it is opt-in: `--lineage` runs ONE whole-file pass and fills `session_kind_first_line` / `session_kind_last_line`, with `lineage_scanned` separating "not asked for" from "no carrier"; without the flag both are null and the row says so. That pass is a depth-1 key walk that never parses a payload, so a nested key is not a field, a torn line simply carries no key, and it contributes NO malformed count - `skipped_lines` keeps meaning the head/tail lines read (R12) and `stats` stays the whole-file census authority. `--lineage` also resolves ①'s `continued_in` over the whole file. COST, measured with hyperfine on the largest transcript of a real corpus (720,932,882 bytes, 21 reps, four arms interleaved round by round, one of them the base binary run twice to size the host): medians 254.5 ms base, 308.4 ms for that same-binary control, 266.4 ms for this tip's default path and 646.0 ms under `--lineage`. The default arm's 1.047x therefore sits INSIDE the 1.212x control band, while `--lineage` is 2.1x the control and 3.68x its user CPU (+314 ms) - which is why it is a flag and not the default. At corpus scale (9,600 files, 6,401,813,687 bytes) the default arm's user CPU is 4.674 s against 4.537 s base and 4.621 s control, again inside the band.
> ⑨ **`plan --audit` QUALIFIES each binding with four facts its own records carry** (claim PLAN-022, resting on PLAN-002, PLAN-005, PLAN-006, PLAN-011, PLAN-014 and PLAN-016). The audit said which plan file a session binds; these four say how much that binding is worth, each as a text block and a JSON field, and each reading a fact on disk rather than deriving one. (a) The slug's CHANGE POINTS - `slug  L<n>  none -> <value>` for the mint, `<a> -> <b>` for a binding that MOVED (JSON `slug_changes:[{line, from, to}]` with `from:null` for the mint, plus `first_slug_line`/`first_slug_utc`/`first_slug_local`). The binding csift reports is the harness's; this says WHEN it became so, which matters because a fork strips the slug from the records it copies and a first own compaction can mint a fresh one. Measured over 9,601 transcripts, 7,810 carry a slug and every one of them has exactly one distinct value and exactly one absent-to-value transition, with zero value-to-value and zero value-to-absent transitions - so a second change point is a shape the corpus has never shown, and printing them is how a reader would ever see it. (b) Whether the bound plan file is on DISK: `[exists]`/`[missing]` beside the path and `plan_exists` in JSON, the SAME `PlanRef.plan_exists` field the forward view prints, so the two cannot disagree. `[missing]` is an ordinary state (PLAN-014: the name is minted at Plan-Mode entry and the `.md` lands only when content is first written), not a fault. (c) A `plan_file_reference` attachment present while NO record carries a slug: plan text with nothing bound to it, which nothing will re-inject, because only the BOUND plan comes back in full after a compaction (PLAN-016). It emits a `plan-unbound-text` row and a warning and is counted in `unbound_plan_text` and in `warnings`. It has ZERO corpus specimens - all 14 `plan_file_reference` transcripts carry a slug - and is kept because the fork path that strips a slug can produce it; the detector reads the attachment's own `type`, so a prose mention never fires it. (d) The first slug-carrying record against the plan file's BIRTH instant (`std::fs::Metadata::created`): `before` is the ordinary order, `after` means this transcript bound a file that already existed, and `unknown` carries its reason in `slug_vs_plan_file_reason` - no file, no timestamp, or a platform that records no birth time - rather than a guessed direction. Cost: ONE walk per in-scope transcript beside the binding resolution the audit already runs, and the slug half goes through the depth-1 key walk rather than a record parse, because a slugged transcript carries the key on most of its records.
...
> ③ **`status` finds the task store a clear left behind** (claim REG-018). The store is named after the id the PROCESS started with, and `/clear` never touches it, so the owner's own id is now the first candidate rather than the only one: then the ROOT of the `cleared_from` chain, then the team file whose `createdAt` sits within 5 s of the registry row's `startedAt`, taking its `name` as the list id because `leadSessionId` can be a throwaway id after a `--resume` launch. A tier is consulted only when the tier above it found nothing, a directory matching no candidate is never read, and every store that answered prints as `tasks store: <dir> (via <candidate>)` with a `tasks_stores` array in JSON - a list recovered by inference never reads as the session's own, and an EMPTY store prints too, marked `- empty`, because the directory existing is the answer to where the list came from. This is the one place v0.12.2 changes a PRE-EXISTING JSON key rather than adding one: a session whose store is found through the `cleared_from` root or the team file now reports its tasks where it reported none, so `tasks` moves from null to an array and `tasks_completed` from null to a count. Over one corpus that is 2 of 80 top-level sessions; the other 78 resolve through the session's own id exactly as before.
> ④ **`plan` states the clear corollary** (claim PLAN-021). The id regeneration calls `forgetPlanSlug` on the outgoing id, so a plan binding does not survive a clear: a clear-minted transcript has none until plan mode is entered again, and `plan` reports none rather than inheriting the predecessor's. No code change - the resolver already keys on the transcript's own `plan_mode` attachment.
> ⑤ **A new leaf `harness.schedule.fire`: the PROMPT a scheduled task fires** (claims CLS-031, CLS-032; §5.1 carries the full leaf text). When a cron entry or a `ScheduleWakeup` timer comes due, Claude Code writes a `system`/`scheduled_task_fire` record naming the instant and then SUBMITS the armed text as its own `type:"user"` record - `isMeta:true`, `promptSource:"system"`, parented to that record's uuid. The text reaches the record VERBATIM: the fire path applies exactly one rewrite, the autonomous-loop sentinel resolution, whose two entry points each read `resolve() ?? resolveFile() ?? prompt`, so an ordinary armed prompt falls through unchanged. With no marker to match, such a record matched nothing and fell into the isMeta exclusion - no label, invisible to a flagless scan, absent from every census, which is why a whole project's scheduled work was unsearchable. The leaf keys on the record's FIELDS instead and its arm sits LAST among the harness marker arms, so a tick that DOES carry a loop marker keeps `harness.schedule.wakeup` / `harness.meta.loop`. An inbound peer message carries the identical `isMeta` + `promptSource` stamp (the submit path puts it on every isMeta submission, not just the scheduler's), so two guards refuse one: the relay framing through the shared `is_peer_message` predicate, and the `origin` object the harness stamps on everything it did not submit itself - the framing guard alone misses a tagless delivery. The INSTANT rides the label zone (`[scheduled fire <when>]`, JSON `scheduled_at`), display-only like `[summarize <dir>]` because it comes from another record; it is joined by `parentUuid` through a per-file index built from the fire lines one more `&&`-gated C-40 conjunction admits (`scheduled_task_fire` AND the key-only `"subtype"`) and DEMOTED back to spine rows once read, so a flagless scan gains no `harness.meta.system` hit from that keep. `Class::ALL` grows to 38 leaves and every search hit's JSON gains `scheduled_at` (null on every other hit). Size, so a reader can weigh it, every figure counted per raw record over the 80 top-level transcripts of one corpus: 171 records carry the fire shape (`type:"user"`, `isMeta` true, `promptSource:"system"`, no `origin`) over 5 files, 168 take the new leaf and 3 keep a tick leaf by arm order; the 465 `scheduled_task_fire` records (top-level `type:"system"` with that `subtype`) sit in those same 5 files. The 7 files that carry the literal ANYWHERE are a different rule and a different number.
> ⑥ **The autonomous-loop markers anchor at CONTENT START** (claim CLS-014). `harness.schedule.wakeup` and `harness.meta.loop` matched their five markers - two headers, two body sentences and the sentinel - ANYWHERE in a body, so a record that merely QUOTED a fired tick was read as one and lost the leaf it had earned: a skill's instruction text that embeds the loop preamble to explain it, a peer message relaying a tick, a compaction summary recapping one. The harness resolver builds the WHOLE delivered prompt and the fire writes it as its own record, so a real tick carries its marker at offset 0; both arms now test exactly that, after the same leading-whitespace trim the resume-prompt arm uses. A genuine tick classifies as before. Over one corpus 11 records carry a loop marker only mid-body, and one of them - the skill's instruction text - was the single record either leaf claimed wrongly. The same trial that found it produced the FIRST specimen of both leaves (one wakeup, two driver ticks), which raises CLS-014 from producer-only to end-to-end.

> **v0.12.1 CHANGE LEDGER (non-breaking; one render fix, one prefilter conjunction, and the survival axis's serial cost removed -- `csift 0.12.1`).**
> ① **An orphan reconciliation pulse renders EVERY task id it closes, and names the kind** (claim BG-020). At the next session start Claude Code reconciles the tasks a previous session left open with ONE `<task-notification>` carrying several `<task-id>` tags, a `__orphan_summary__:<kind>` sentinel that names no task, and status `stopped`. The background scan already joined every id; the RENDER read only the first, so a pulse closing five tasks showed one and hid four, and the sentinel could take the id slot outright. `model::section_task_ids` now splits one section's tags by the shared `__orphan_summary` prefix Claude Code gives its own scan markers: the real ids render comma-joined in the id slot of `[<kind> <task-id>[, <task-id>...] <status>]`, the sentinel's kind renders as a trailing `(orphan reconciliation: <kind>)`, and `AutomationTrigger::task_id` resolves to the first REAL id. `automation_label_for_section` is the one helper behind `search`, `show`, `list` and `verbatim`, so every record surface follows. `search` hit rows gain `task_ids` (array) and `orphan_kind`, carried per SECTION because a batched record's sections name different tasks. Rarity, so a reader can size the change: over one corpus, of 3076 notification sections on the top-level transcripts, 2 carry more than one `<task-id>`, both orphan reconciliations, naming 3 and 2 real ids in 2 sessions.
> ② **The D7 compaction-boundary keep is a CONJUNCTION** (claims CMP-002, CLS-021, QT-SYS-001; §4.7 carries the counting rule, §7d the prefilter statement). Behind the same `-t` gate the keep now wants the key-only bytes `"subtype"` beside the literal `compact_boundary`, the literal tested first so the second `memmem` runs only on the few lines carrying the rare word. The literal alone was not the discriminator, because a payload can write that word in prose. A refused line is still lifted to a structural spine row, so the survival chain reads through it, and an explicit address still fetches it whole. What this closes is a queued prompt whose content names the literal: it used to be admitted on a flagless scan, classified `user.queued`, emitted and censused, while the same run's honest-empties note reported that leaf as unscanned.
> ③ **The survival axis's serial cost is removed; NO output byte moves** (C-41; §7g carries the measurement, claim REC-104 stands unchanged). Through 0.12.0 every full-scan surface put the chain-structural spine rows of the lines its prefilter dropped in the SAME vector as its own records, so each element was sized to a `Record` and the whole vector was moved out of the scan and then split. The two kinds now ride separate output streams from the scan down (`parse::scan_lines_parallel_split`, one `SplitVerdict` per line, the same chunking and malformed accounting), the spine row is its own narrow type (`parse::SpineRow`, 72 bytes against `Record`'s 1184, its width asserted by a unit pin), and the consumers merge the two back by line into a BORROWED file-order row list that is the index space every chain answer is addressed by. The chain reads both kinds through one shape, `model::ChainNode`, whose message-derived accessors answer the same `false`/`None` a message-less record gave. This is a performance change only: stdout, stderr and exit code are byte-identical to the 0.12.0 binary on 851 (command, file) pairs over the 76 top-level transcripts of one corpus, and on a further 1368 from a supplementary sweep of 18 more command shapes over the same files, 2219 pairs in all.

> **v0.12.0 CHANGE LEDGER (BREAKING; the survival axis, the resume repair pair, the boundary's survivor fields and the background entrances -- `csift 0.12.0`).**
> ① **THE SURVIVAL AXIS: Claude Code's own chain rule replaces three opener heuristics** (claims MISC-034, MISC-035, MISC-036, MISC-037, MISC-038, MISC-039, MISC-041, MISC-043, REC-103, TURN-026, TURN-032, CMP-019, CLS-030). Claude Code does not reconstruct a session by reading its jsonl top to bottom. It streams every `user`/`assistant`/`attachment`/`system` line carrying a string `uuid` into a map where the LAST occurrence of a uuid wins (MISC-034), picks ONE leaf through four gates (MISC-035: a `last-prompt` line's `leafUuid`, recorded with every write batch and wiped by a later `compact_boundary`; `explicit:true` with a null leaf clearing the set; a recorded leaf that is a proper ANCESTOR of the file-order-last record yielding to that record; a leaf naming no record in this file falling back to the tail, never to the next-newest `leafUuid`), and walks `parentUuid` from it to the head, reading that field and nothing else (MISC-036). `logicalParentUuid` is never read on that path, so a `compact_boundary`, whose own `parentUuid` is null, ends the walk. Only what the walk reaches is the conversation. Two passes bracket it: BEFORE any leaf is picked, the records a `compact_boundary` names in `preservedMessages` / `preservedSegment` are re-parented onto its anchor and everything below is cut (MISC-037); AFTER the walk, membership widens past plain ancestry by two more (MISC-038: the same-`message.id` assistant siblings of a chain assistant, plus the `tool_result` carriers parented to them, and every non-conversation descendant of the leaf). `model::Chain` mirrors all of it hop for hop, adding only the 5 s same-`isSidechain` parent repair and the cycle guard, and yields `Survival::{Live, PreCut, Abandoned{root}}` per record. csift carried three special cases that approximated the same answer - the same-parent draft rule, the replay-copy collapse and the boundary's implicit cut - and not one of them could see a turn the operator rewound past, or say which child of a fork the conversation continued from. All three are gone; the rule itself is the replacement.
> ② **What the axis changes for a reader.** An ABANDONED record sits outside turn numbering, is skipped by a bare `-t user` / `-t agent` / `-t harness` (survival is the THIRD exclusion axis beside leaf visibility and record-level delivery, §5.6) and is reached by a glob or an exact leaf path with an `[abandoned]` / `[rewound]` / `[replay copy of L<n>]` marker in the label zone. An abandoned OPENER carries ONE leaf, and the discriminator is a DESCENDANT: `user.unsent` when no assistant record hangs below it, the new `user.rewound` when one does (TURN-032, over the corpus's 910 shared-parent opener pairs: 858 earlier members with no assistant descendant, 52 with one, 0 of the 910 on the final chain). Both leaves are retroactive for the same reason `user.unsent` always was: nothing on the record marks it, the fact lives in the DAG around it (TURN-026, MISC-041). The replay collapse keeps its EFFECT and flips its DIRECTION (CMP-019 read against MISC-034): the loader's map holds the LAST line carrying a uuid, so that line is the survivor for opener treatment and for addressing by uuid, while the earlier line renders `[replay copy of L<n>]` and stays searchable and counted. Turn numbering therefore shifts on any transcript carrying a rewind or a replayed block, the v0.5 and 0.9.2 precedent. The census over the reference corpus CONSERVES the opener count, 3,650 before against 3,650 + 7 after: seven records that used to count as turn-opening `agent.communication.signal` are now `user.rewound`, so that leaf reads 397 where it read 404. Nothing was dropped; the seven moved leaf. Every count is disclosed - the text footer and the JSON summary carry `abandoned_records`, `rewound_turns`, `replay_copies`, `boundary_cut_line` and `leaf_source`, and each hit carries `survival`, `abandoned_root_line` and `replay_copy_of`.
> ③ **TWO fail-open guards, each with its own measured cause.** csift's §7d prefilter keeps only role-bearing lines, and a chain built from those alone resolves nothing: 1 of 30,613 conversation records reachable on one real session. So every surface splices back the STRUCTURAL row of each line its own prefilter dropped, through `parse::spine_record` - a depth-1 key walk lifting `type`, `uuid`, `parentUuid`, `timestamp`, `isSidechain` and the boundary fields into an otherwise EMPTY record, never the payload. The SECOND guard is a FLOOR, and its cause is where csift's own walk stops. A `compact_boundary` carries a null `parentUuid` (260 of 260 boundary records over every transcript under one projects root), so the walk's null-parent arm ends the walk ON the boundary exactly as Claude Code's does, and everything physically above it is a BLIND REGION csift did not resolve. The walk's terminating record therefore floors the verdict: an off-chain record above it is `PreCut`, NEVER `Abandoned`, and inside that region the measured same-parent opener rule still marks a draft, so draft detection never depends on the chain reaching that far. What csift adds above a cut is not a crossing but a refusal to discard: Claude Code drops that history, csift keeps every record the walk did not reach, flagged, and every selector still reaches it, because what the model saw AT THE TIME is the question an archive answers. The `logicalParentUuid` arm in the walk is a GUARD for one shape only, a boundary whose own `parentUuid` is non-null and resolves to no record, and the corpus holds zero specimens of it: renaming the field on five real compacted transcripts of 9 to 21 MB leaves the per-hit `survival` census, the `stats` chain totals and the search-summary disclosures byte-identical. The blind region is not a partial record set either: no admitted record's `parentUuid` names a progress or unparsed line (12 transcripts probed, empty census). MISC-043 records what the guard would meet if it ever fired: 52 of 255 `compact_boundary` records, across 14 of the 21 compacted transcripts among 77 top-level ones, name a `logicalParentUuid` matching no `uuid` on any line of their own file, the pre-compaction records having been deleted from disk by the pre-boundary deletion path.
> ④ **C-34: the resume repair pair gets its own harness leaves, and one leaf is RENAMED** (claims CLS-013, CLS-028, CLS-029). When Claude Code resumes a transcript whose last non-system/non-progress record is a `type:"user"` one -- an esc-recalled draft never resent, or any prompt that drew no reply -- the LOADER repairs the dangling tail before handing the conversation to the model. It appends an `isMeta` user record reading `Continue from where you left off.`, and it splices a stand-in assistant record reading `No response requested.` in right after the trailing user record, both with one identical timestamp. **The model receives both**: the request assembler's drop predicate reaches neither, because its `<synthetic>`-model arm also requires `isApiErrorMessage`, which the loader does not set. Only Claude Code's own `--reply-on-resume` flag suppresses the splice. Corpus, one event per marker record over the uuid-named top-level transcripts: 75 scanned, 10 carry a marker, 8 prompts + 18 placeholders = 26 events.
> ⑤ **`harness.schedule.continuation` -> `harness.resume.prompt` (BREAKING).** Same predicate, same records, new path. The record comes from the resume loader and never from the scheduler, and the sibling placeholder needed a family to sit in; `harness.schedule` keeps only `wakeup`. The retired spelling is a HARD parse error naming its successor -- the v0.6.3 precedent for the pre-v0.5 flat values, not a compat shim. Anything keying on the old leaf path, in a script or a saved query, must be updated.
> ⑥ **`harness.resume.placeholder` is new, and it is a correctness fix, not just a label.** Those 18 records classified `agent.message` before -- which asserts the assistant wrote a sentence no model produced. The predicate is a 1:1 port of Claude Code's own recogniser (assistant, `isApiErrorMessage` falsy, `message.model` the `<synthetic>` sentinel, single-text content exactly the sentence); the model sentinel is the load-bearing half, so a genuine reply that happens to say the same words keeps `agent.message`.
> ⑦ **The pairing is a FACT on the hit, not a second leaf.** ONE writer produces every placeholder and its gate is only "the tail is a user record", while the prompt's gate is the tail classifier's interrupted-turn verdict -- so the same splice lands ALONE after an interrupt marker, a `<local-command-stdout>` echo or a slash wrapper. Measured: of 18 placeholders, 8 have a repair prompt as their `parentUuid` and 10 do not (7 `[Request interrupted by user]`, 1 `...for tool use]`, 1 stdout echo, 1 slash wrapper). Splitting one producer across two leaves would have hidden that, so both forms carry the leaf and the verdict rides beside it: JSON `resume_paired` (`true`/`false`, `null` on every non-placeholder hit) and the label-zone marker `[paired]` / `[unpaired]`.
> ⑧ **Neither half opens a turn, and `status`/`wait` walk past both.** The prompt is `isMeta` (already excluded from every opener case); the placeholder is an assistant record. The `last` section needed the explicit skip for the placeholder -- it is an assistant record with real text, so a resumed session reported `No response requested.` as its newest reply.
> ⑨ **The prompt predicate requires `isMeta`, and that is a safety property, not a detail.** Claude Code's own recogniser tests the flag, and the injecting call site is what stamps it. Without the same requirement a person who merely BEGINS a real message with the sentence -- `Continue from where you left off. and also fix the tests` -- would have their prompt reclassified as machinery and dropped from turn numbering: the C-28 delivery error in reverse, applied to a human. With it, that message stays `user.message` and opens its turn, and both of this leaf's exclusions hold THROUGH the gate `is_genuine_user` already reads. csift does not apply the recogniser's third test (`promptSource` absent), so its set is a superset by exactly the records carrying that field; none exists in the corpus, and the gap is recorded as residue rather than guessed at.
> ⑩ **C-33: the compaction boundary's survivor fields, and the two summarize modes are distinguishable** (claims CMP-001, CMP-002, CMP-020, CMP-021). `compactMetadata` was always more than the four scalars csift rendered, and two of the extra keys say what a compaction KEPT. The boundary excerpt now names every field the record carries, in a fixed order and only when present: `[compaction boundary: trigger=… preTokens=… postTokens=… durationMs=… messagesSummarized=… cumulativeDroppedTokens=… preserved=<N uuids, M allUuids, anchor <first8>> segment=<head first8>..<tail first8>]`. `cumulativeDroppedTokens` is the running total of context tokens every compaction of the session has dropped; `preservedMessages.uuids` is the on-disk subset of what was kept and `allUuids` the in-memory superset, which is why an `allUuids` entry can resolve to no line (2 of 19 on the specimen). The one-line excerpt COUNTS those lists and prints first-8 handles, because a boundary line has to stay scannable; JSON `compact_metadata` carries the object verbatim, uuid lists included. **A boundary carrying only the four original scalars renders byte-for-byte what it always did** (pinned by a unit test and an e2e test).
> ⑪ **The `/rewind` summarize modes get a name.** Claude Code compacts three ways and writes the same two records for all three, so an ordinary `/compact` and a `/rewind` → "Summarize up to here" were indistinguishable on csift's surfaces. They are separated by ONE key, and it sits on the SUMMARY: the writer emits `summarizeMetadata:{messagesSummarized, userContext?, direction}` INSTEAD of `isVisibleInTranscriptOnly` (a genuine either/or in one ternary), with `direction` reading `"up_to"` or `"from"`. csift derives a MODE - `compact` | `summarize-from-here` | `summarize-up-to-here` - and prints it on BOTH records: JSON `mode` on the `search` hit, the `show` record row and the `verbatim` `compaction_boundary` row, a `[summarize <direction>]` label-zone tag on the summary's text line, and a `· summarize <direction> ·` segment in the `verbatim` banner. The BOUNDARY has no direction of its own, so it learns its mode by pairing with the summary that follows it (`model::SummarizeIndex`, one pass per file, carried on `ClassifyCtx` beside the spawn lookup); an UNPAIRED boundary and an unmodeled `direction` both read null rather than a guessed `compact`. The pairing is built from the WHOLE parsed record set before any `--turn` window, `--max-count` cap or `show` address narrows the output, so it is window-independent by construction: `show --line <the boundary>` still reports the mode, and only the file itself (no summary after the boundary, or a second boundary first) leaves one unpaired. `userContext` is written from a possibly-undefined value and is absent from the only corpus specimen, so the discriminator is `direction` alone, never a three-key shape.
> ⑫ **A summarize IS a compaction, and `verbatim` treats it as one.** The gesture clips turns exactly like an auto-compact - the summariser reads the in-memory message array and the file keeps every record it dropped (claim CMP-020) - so reconstruction is unchanged and the mode only labels the event. The label-zone tag needs no synthesized-marker registration (it is display-only, like `[narration summary]` and `[not delivered]`), and the boundary excerpt's new fabricated bytes are already covered by the `-t`-gated `compact_boundary` marker, which is a raw substring of the same line.
> ⑬ **The `/rewind` menu's other rows, recorded because their on-disk trace is EMPTY** (claims REC-101, FH-051, FH-052, REC-102, MISC-032, MISC-033). "Restore conversation" truncates the live message array in memory and appends nothing - none of thirteen record-writing names occurs anywhere in its body, and 0 of 30,658 `last-prompt` lines in the corpus carry a `rewound` or `explicit` flag. "Restore code" writes only the working tree and can DELETE a file whose backup is null; the reducer arm it dispatches bumps a counter and writes neither a `file-history-delta` nor a `file-history-snapshot` line, which the corpus confirms by key set (2,181 delta lines and 4,359 snapshot lines, one key set each, no restore marker). The only trace that follows is the next turn's `edited_text_file` attachment, an mtime-gated re-read carrying a DIFF, whose snippet is emptied past a 16,384-character budget. And the branch a rewind leaves behind stays on disk in full: 1,481 abandoned roots in 44 of 75 transcripts, 11,142 records, 422 tool calls - which is why csift replays FILE ORDER and never a surviving chain when it attributes a mutation.
> ⑭ **`show --branch-points` names the live child.** The fork rows are unchanged - every record with 2+ conversation children, children with lines and timestamps, ranked by the widest inter-child gap - but the question the v0.8.0 ledger left open is now answered by Claude Code's own conversation chain (`model::Chain`), not by a sibling-shape heuristic. Each fork prints `live child: L<n>` (or `live child: none` when the whole fork sits off the surviving conversation, or `live children: L<a> L<b>` on the rare row the chain's membership passes keep two of), and every child carries a verdict: `live` · `rewound` (it was answered, and the conversation was rewound past it) · `draft` (nothing ever answered it) · `abandoned` (off the chain, mid-branch) · `pre-cut` (above a compaction boundary, where the loader stops and csift keeps reading). The PARENT lookup widened at the same time: a fork parent is often not a conversation record at all - a prompt submitted after a SessionStart hook is parented to that hook's `attachment` line - so the scan now lifts the structural spine row of every line the role prefilter drops (`parse::spine_record`, the same splice `search` uses), locates the parent among them and prints its line AND its type. `(parent line not located)` is retired: a uuid that names no line in the file reads `parent uuid not in this file`, which is the only case left. JSON branch-point rows gain `parent_line`, `parent_type`, `live_child_line` (null unless exactly one child is live) and, per child, `survival` (`live` | `pre-cut` | `abandoned`) and `verdict` (the five values above); `line` keeps its envelope meaning and equals `parent_line`, since on this row the record IS the fork parent. Measured over the reference corpus, one row per record with >=2 conversation children: 745 fork rows over 1,799 children across 33 of 75 top-level transcripts, and the parent located on 745 of 745 - a `system` record on 539 rows, an `assistant` on 94, a `user` on 68, an `attachment` on 44, so 583 of 745 (78%) are lines the role prefilter never keeps, exactly the population the old lookup could not resolve. Those figures held across two passes; the live/abandoned SPLIT of the same children did not, and that is the expected direction rather than noise. Appending to a transcript moves its leaf, the leaf decides the chain, and the chain decides which child is live - while the fork itself, a plain parentUuid fact, does not move. Two passes read 0 live children on 679 then 686 rows, 1 on 65 then 58, 2 on 1 both times; child verdicts read draft 871 / pre-cut 824 / live 67 / rewound 36, then 873 / 831 / 60 / 35. The shape is what is stable: most forks have NO live child because they sit above a compaction cut, and exactly one row has TWO, the membership widening keeping a same-`message.id` assistant sibling.
> ⑮ **`stats` counts LIVE turns, and discloses what left the numbering.** `stats` splices the spine exactly as `search` does, so its `turns` is now the count of numbered LIVE turns - the same numbering `search` prints as `·tN`, `show --turn` addresses and `search --count-by turn` enumerates as rows - instead of a count derived from role records alone. A `--turn` window resolves against that live numbering too. Three whole-file chain totals ride along, per session row and in the scope TOTAL: `abandoned_turns` (turn openers the conversation chain no longer reaches), `rewound_turns` (the subset of those that drew a reply before the rewind) and `replay_copies` (lines a compaction re-anchor re-appended). They are FILE facts like `lines` and the `line_types` census and are never windowed by `--since`/`--until`/`--turn`, because an abandoned opener carries no turn index to window on. The text block gains one line, printed only when there is something to say: `chain  abandoned turns N (M rewound) · replay copy lines K  (whole-file facts, outside \`turns\`)`. The line-type census is unchanged and stays the exact corruption-census authority: a non-candidate line is still fully validated by its own type probe, and the spine row is lifted OUT OF that same parse (`parse::line_type_and_spine`) rather than by a second byte walk: on the corpus's largest transcript by bytes (720 MB, 91,975 lines of which 49,902 are attachments) the UNFUSED shape - a narrow type probe plus a separate `spine_record` walk - costs about 40 ms of user CPU more than the fused one, and the SIMD in-string hop inside `spine_record` is worth a further 136 ms to `show --branch-points` on the same file. Method for both: medians of 16 and of 20 runs per arm, arms interleaved forwards and backwards each round, user CPU from `getrusage` deltas, a same-binary control drifting 0.3-0.6%. Parity receipt over the reference corpus, one observation per transcript: `stats` `turns` equals the `search --count-by turn` row count on 75 of 75 top-level transcripts (6,323 live turns at one pass, 6,326 at a later one - the corpus is live, and it is the AGREEMENT that is the claim, not the sum).
> ⑯ **`verbatim`, `files`, `recover` and `image` join the same numbering, through `model::ChainView`** (claims CMP-020, FH-015, FH-021, QT-025, REC-025, MISC-030, REC-075). `search`/`show`/`stats` splice the spine rows into their own record list; these four index everything by their OWN record positions and cannot, so `parse::parse_candidates_with_spine` lifts a dropped line's structural row in the SAME pass (one parse per line, never two) and the view merges the two slices by line number, runs the ONE chain and answers back in the surface's own index space: `survival(i)`, `stamp(i)` (`TurnStamp::{Live(n), Abandoned{root_line}}`), `order_turn(i)`, `turns()` and `groups()`. A turn number therefore means the same thing on every command: a `verbatim` turn 7, a `search --count-by turn` row `t7`, a `show --turn 7` and a `files --turn 7` name one turn (checked per session over 75 top-level transcripts: 64 agree, 0 disagree, 11 too small to compare). `image --turn` joins them; it had stayed on the older, more conservative grouper. The agreement was re-established, not assumed: on a 19-transcript `files`-to-`search` join, one observation per paired row, a build that had wired the axis into only some surfaces read 2,085 mismatched turn stamps, 23.1% of the paired rows, where 0.11.1 read 0 and 0.12.0 reads 0. This REPLACES the interim state the chain landed in, where these four saw a role-only DAG and every record above a compaction cut read abandoned.
> ⑰ **`verbatim` replays LIVE turns only - a correctness rule, not a display choice.** A turn the operator rewound past, or a prompt recalled and re-typed, is counted in a per-session footer that carries the exact `csift show @<id> --line N` fetching it, and is never re-emitted; JSON adds `abandoned_turns` to the summary and `survival` to every unit. The reason is claim CMP-020: the compaction summariser's input is the in-memory message array, so it never saw that turn either, and replaying it would put text no model read into a reconstruction of what a compaction clipped. `--slice` follows the live turns. Reconstruction still crosses a compaction CUT where Claude Code's own loader stops - those units read `survival` `pre-cut` rather than being dropped, because what was written was the conversation at the time.
> ⑱ **`files` and `recover` KEEP every abandoned row and event, and MARK it - disk truth is FILE ORDER.** An Edit on a branch the operator later rewound past LANDED on disk, so the row stays where the file put it; its turn slot reads `abandoned (root L<n>)`, the text row carries `[abandoned]`, and JSON nulls `turn_index` beside `survival` and `abandoned_root_line`. `files --by summary` counts the abandoned share ALONGSIDE the total (`N op(s) (M from abandoned branches)`, JSON `abandoned`) rather than subtracting it, and `recover` discloses how many replayed events came from abandoned records. A `--turn` window on these two admits such a row by the LIVE turn it physically follows (`order_turn`): a window over a span must not silently omit a write that happened inside it. The conversation surfaces make the opposite call for the same reason.
...
> ㉑ **Cost, stated in numbers.** The spine pass reads payload bytes it deliberately never parses, and that is the whole of the added cost: a probe build returning from `spine_record` before it walks puts every slower command back at the pre-axis baseline within noise, while the merge, the chain and the turn grouping measure at zero. The scale is the point - on a 397 MiB transcript the pass builds 100,387 structural rows against as few as 183 records the command itself wanted. Measured against 0.11.1 on that transcript, medians of 21 runs per arm on an interleaved CPU-time driver with rotated cells and a same-binary control drifting under 2%, the ratios quoted being wall clock: `image` 1.74 to 1.77x, `verbatim` 1.35x, `show --branch-points` 1.54x, `recover` 1.19x, `stats` 1.17x, and a heavy `-t user.unsent` search on a 721 MB transcript 1.17x; `files` reads 0.84x, FASTER, because the view replaced four per-file chain builds with one. The cost is not where a reader would expect it. The byte walk runs inside the parallel scan, so on `image` it is 103 ms of CPU and 17 ms of wall, and building the row is 25 ms of CPU and under 2 ms of wall. What lands on the clock is serial: two bulk moves of a row vector at 1,184 bytes a row, and the chain build itself, which grows from 1 ms over 183 nodes to 46 ms over 100,570. FOUR levers were measured and rejected on the way to that finding. An early stop in the key walk is impossible, because Claude Code writes `uuid` and `timestamp` AFTER the payload key on 662,470 of 673,710 admitted lines (claim REC-104). A reverse scan from the line end lands within one percent of the forward walk on every shape. A leaner row buys about 1.5% on `search` and half a percent on `image` and `verbatim`, because its construction was never the cost. Two reductions ARE in: the SIMD `memchr2` jump over string interiors, and the fused `line_type_and_spine` entry that lifts the row out of the parse `stats` already runs. The remaining headroom is in the serial carry, and taking it means the spine rows must stop sharing a collection with full records: scoped and measured at a lower bound of 13% on `image` and 8% on `verbatim`, and shipped as a later patch, not as a last-minute change to the seam this release just settled. Section 7's structural contract is otherwise untouched - mmap, the memchr line scan, the byte prefilter, the tail reads, rayon and the whole-file gate all work exactly as they did.
...
> ㉓ **C-39: a gated attachment the scan cannot search is still a node of the chain.** Two candidate needles are DEFAULT-ON, and both admit `attachment` lines a flagless scan has no leaf for: the v0.11.0 `csift-channel v1` literal, and the D7 `compact_boundary` value substring, which keeps any payload that merely uses the word. v0.11.0 dropped such a record from the record list before the match phase so it could not leak out under a gated leaf, and that deletion is a chain hole: the survival axis walks `parentUuid` THROUGH attachment records, so the walk below the hole cannot resolve its parent, the 5 s same-`isSidechain` repair only bridges a gap that narrow, the walk ends, and its terminating record becomes a FLOOR that paints everything physically above it `PreCut` - or `Abandoned` where the hole sits off the main line. The record is now DEMOTED instead: `scan::search_one_file` reduces it in place through `parse::spine_from_record` to exactly the structural row `parse::spine_record` lifts off the same line (pinned field for field by a unit test), so it is invisible to every emission pass - no hit, no sibling, no census key, no turn opener - and visible to `model::Chain`. Nothing an explicit query could reach changes: an address or either attachment flag still admits the record whole, a real delivery is still kept whole under `agent.communication.channel`, and the two v0.11.0 leak pins (an `edited_text_file` snippet quoting the header, a hook context merely mentioning it) still emit nothing on a flagless scan. **Measured over the 76 uuid-named top-level transcript files of a real corpus, one observation per file.** 173 lines on 5 of them were admitted-then-deleted, every one by the D7 needle and none by the channel needle (the corpus holds no delivery at all). Chain diagnostics from the JSON summary, flagless scan versus `--attachments` on the same binary: 3 of 76 disagreed before (`abandoned_records` 26 against 1031 on a 7.2 MB transcript, 63 against 59 on a 162 MB one, and a 160 MB one reporting `boundary_cut_line: null` where the gate-open arm named its newest boundary), 0 of 76 disagree after. On the 7.2 MB file - which carries no `compact_boundary` record at all - 1,190 of 1,246 emitted rows read `pre-cut` before and 0 do after, and `search` versus `show --line` on a 12-line sample of them agreed 0 of 12 before and 12 of 12 after, because an address opens the gates and `show` was reading the same lines correctly the whole time.
> ㉔ **C-30: an inbound `<cross-session-message>` was invisible** (claim TURN-031). A message sent from one Claude Code session to another lands in the RECEIVER's transcript under a THIRD peer framing, distinct from `<teammate-message>` and `<agent-message>`: a `type:"user"` / `role:"user"` / STRING record carrying `isMeta:true`, `promptSource:"system"`, `userType:"external"`, `queueSkipAttachments:true` and a top-level `origin` object `{kind:"peer", from, verifiedPeerPid, msg_id?, name?, fromMode?, body?}`, whose content is the usual relay preamble, then `<cross-session-message from="uds:…" from-session=… hop-chain=… from-name=… from-mode=…>` (five attributes, fixed order, all conditional), the body, `</cross-session-message>`, and the security footer. `is_peer_message` accepted only the other two, so the record matched NO marker, its `isMeta` gate emitted the EMPTY label vector, and the delivered message had no leaf, no census row, no `-t` result and no rendered `show` address - only `show --raw`. It now classifies `agent.communication.inbox` and opens a turn like the other two framings; the direction is `from-name` where present, else `from` (the first is what a human named the sending session, the second a socket or bridge address). The close tag joins the section-boundary set, so a BATCHED record's later sections are still recognised, and detection stays boundary-anchored - this repository's own documentation quotes the literal tag, and Claude Code's own reader anchors the same way.
> ㉕ **The queue rider is refused by construction.** A `queue-operation` `enqueue` line carries the identical framed content one line before the user record, and `user.queued` is defined as "the queue line carries the HUMAN's text" - the same `is_peer_message` predicate decides both, so folding the third framing in stops the rider being counted as typed text without a second rule that could drift. Measured over the top-level transcripts: 24 `enqueue` + 6 `remove` lines carry `<agent-message>`, 1 `enqueue` carries `<cross-session-message>`.
> ㉖ **`origin.name` / `origin.body` are preferred over a tag scan, but only for a single-section record.** Claude Code fills those fields by re-parsing the framed string, so they agree with the tag's own attributes by construction; reading the structured pair survives a body a tag scan cannot bound. One `origin` describes ONE message, so a record batching several peer sections falls back to each section's own attribute rather than stamping the first sender onto all of them.
> ㉗ **`record_text_sections` renders the peer BODY, not the section slice.** `inbound_comm_preview` (`verbatim`/`list`) already stripped the wrapper tags, the relay preamble and the security footer; the `search`/`show` render emitted the whole section including the open and close tags, so the same message read one way in one command and another way in the next. Both now render the same tag-stripped body (claim TURN-015). Consequence for a matcher, and it is a LOSS, not a flag away: the tag and its attributes are no longer part of the rendered text, and rendered text is the only thing `search` matches — so `search 'teammate_id='` now returns zero. **`--raw` does not recover it.** `search --raw` is OUTPUT-only: the per-file mmap backfill onto `Hit.raw` runs AFTER the match phase, over the lines of hits that already matched, so it changes what a hit PRINTS and never what the matcher SEES (scan.rs, the `if args.raw` backfill; the same is true of `--format json`'s `raw` field). What survives: the peer's own words match as before; the direction (`from ⇨ to`) still carries the sender; and the verbatim line of a record you can ADDRESS is reachable with `csift show @<id> --line N --raw`, which needs no match at all. The rendered body stays a VERBATIM substring of the source line, so the §7d literal prefilter and the §7f whole-file gate hold with no synthesized-marker registration.
> ㉘ **The fetch domain now matches the refetch law.** `classify` deliberately yields NO label for some real message records - an `isMeta` pseudo-turn matching no harness marker (§4.2's M2b rule: better silent than mislabeled `user.message`), a block record with no text - and `show --line`/`--uuid` on such a record answered `no such record(s)` on a line that is plainly there. An ADDRESSED record with no modeled leaf now renders as ONE UNLABELED unit: `Hit.class` became `Option<Class>`, text prints `? (no label)` (the `?` marker is new, beside `◂ ▸ ⚙ ·`), and JSON carries `"label": null` with `"labels": []`. The fallback is gated on the address, so a SCAN emits nothing new and no census, `-t` result or count moves; a line that is no record at all (a session-state cache line, a content-less queue dequeue, a torn line) is still a hard miss pointing at `--raw`.
> Taxonomy: 35 -> 37 leaves.

> **v0.11.1 CHANGE LEDGER (non-breaking surfaces + ONE correctness fix that SHIFTS TURN NUMBERS -- `csift 0.11.1`).**
> ① **CORRECTNESS: a replayed record is no longer read as a resend, and turn numbering shifts where that happened** (claim CMP-019). A compaction RE-ANCHOR re-appends a contiguous block of already-written records with their uuids PRESERVED - and a turn-opening user record in that block keeps its `parentUuid` too, differing from the original in exactly one top-level field, `promptId`. The C-18 draft rule groups openers by `parentUuid` and keeps the last, so it read the copy as a later sibling and marked the ORIGINAL - a message that was sent and answered - as a `user.unsent` draft, dropped it from turn numbering, and (from the C-27 diff line) rendered it as differing from the sent message in 0 chars. Measured: 7 genuine messages mislabeled in one session; whole-file, 464 uuids appear twice, 462 with the same `parentUuid` (attachment 289, assistant 117, user 44, system 14). `model::collapse_openers` now decides both opener corrections in ONE walk and collapses same-uuid openers under one parent to their FIRST occurrence: the first keeps its label and its turn; the copy supersedes nothing and OPENS nothing, but stays a turn MEMBER, so it is still addressable and consistent with the replayed assistant and attachment records, which have always rendered at both of their lines. **What is de-duplicated is the OPENER treatment - the draft label and the turn - and nothing else: a replayed block's records are still counted and matched at BOTH lines.** In the measured specimen `-t user.message --count-by session` reads 666 where 0.11.0 read 659, the seven originals AND their seven copies, where 0.11.0 counted the copies and labelled the originals `user.unsent`. That is the SAME double-count law §3.5 states for a clone, one file down instead of across files: a spanning surface counts duplicated records once per line until the duplicate is scoped away, and csift discloses the shape rather than silently picking a copy. **Turn numbers therefore shift on any transcript carrying a replayed block** - the same class of change as the v0.5 slash-wrapper fix and the 0.9.2 draft work, and the same rule applies: a `tN` you copied from an earlier csift may name a different turn, so re-read the header rather than reusing a number. `superseded_draft_indices` / `superseded_draft_map` / `group_turn_indices_deduped` all derive from the one walk, so `search`, `show`, `files`, `recover` and `verbatim` cannot drift apart on it.
> ② **A draft whose text did not change says so.** When the diff is exactly 0 and exact - a recalled message resent unedited - the line reads `identical to the sent message` instead of `differs from the sent message in 0 chars (0.0% of the final)`, which invited the reader to hunt for an edit that is not there. JSON is unchanged (`diff_chars: 0`, `diff_exact: true`).
> ③ **C-27, the unsent diff line.** A RENDERED `user.unsent` draft now states its distance from the message that replaced it, in text as one line under the draft header (`differs from the sent message in N chars (P% of the final): may carry an addition or a correction`) and in JSON as `superseding_line`, `superseding_uuid`, `diff_chars`, `diff_pct`, `diff_exact` - on the `search` exchange row where `superseded_draft` already lives, and on the `show` record row (null there on every other row). N is INSERTIONS PLUS DELETIONS of a shortest CHARACTER-level edit script between the draft's reconstructed text and the final's - the characters covered by the differing regions - never a length difference, so a mid-text replacement counts as the work it was; P is N over the final's character count and can exceed 100 when the draft was the longer text. Both sides come from `Record::reconstructed_user_text`, the engine the draft's own hit renders through, and both are compared as chars so a multi-byte body is never split. The engine is `src/chardiff.rs` (a char-level Myers O(ND) walk, no new dependency) and the §7 contract is what shapes it: the cost is O(N*D) and the greedy loop alone walks about D^2/2 diagonals, so FOUR bounds apply in order - the common head and tail are stripped, one linear subsequence test settles the dominant corpus shape exactly and for free (a short draft replaced by a longer message that still contains it), the residual length difference is a STRICT floor whenever that test fails and is the floor every giving-up arm reports (never the weaker `MAX_DIFF_CHARS` = 200,000 cap), and what survives walks under a step budget. Every inexact result is a proven strict floor rendered as `more than N`, never an estimate. The diff runs only at EMISSION, for a draft that produced a hit, and only when exchanges will be rendered: `-c` / `-l` / `--count-by` print no exchange and pay nothing. Measured over the whole corpus AFTER the ① guard, on a population of 887 drafts (the live corpus gains drafts as sessions run, so the number a re-run sees drifts up): 877 exact and 10 reporting a floor between 3,813 and 9,360 characters, each within a few percent of the true distance; the largest draft (237,434 chars) diffs in 0.755 ms, and both `search -t user.unsent --max-count 0` and a broad `search the` measured within noise of the previous release.
> ④ **C-26, the retroactive label, stated as such** (claim TURN-026). `user.unsent` is a statement about a LATER record and can only ever be assigned after the fact. CENSUSED OVER THE FULL POPULATIONS, on csift's own output and after the ① guard: all 886 superseded drafts in 28 sessions, each joined to the survivor its own `superseding_line` names (so no forward scan, no cap, no locally defined opener predicate, and no unpaired draft), against all 2,595 `user.message` records (2,567 exchange rows) of the same sessions. The draft and its resend carry the same top-level key set on 852 of 886 pairs (13 distinct key sets on the draft side, 15 by pair union; the one-sided keys are `imagePasteIds` 29, `origin` 5, `queueSkipAttachments` 1), and among shared keys `promptId`, `uuid` and `timestamp` differ on all 886, `message` on 862, `promptSource` on 28, `origin` on 15, `version` on 1 - identity and time apart, the two records are the same shape. Nor does the window between them mark anything: it is almost never empty (1 of 886) and holds ordinary end-of-turn telemetry, with rates (drafts/control) `file-history-snapshot` 99.3/91.2, `last-prompt` 64.4/53.7, `permission-mode` 64.3/53.7, `mode` 64.3/53.4, `ai-title` 64.3/53.8, `hook_additional_context` 56.4/51.3, `agent-name` 42.0/37.8, `total_tokens_reminder` 27.8/29.7 and 40-odd further shapes below 11% on the draft side. Eight shapes read ZERO on the control side, and every one of those zeros is the control window's assistant cut rather than a fact about drafts: with the window cut only at the next opener, `system/stop_hook_summary` goes 0 -> 85.2%, `system/turn_duration` 0 -> 65.8%, `attachment/queued_command` 0 -> 15.0%, `attachment/batching_reminder_sent` 0 -> 7.9%, `attachment/bash_output_audience_note` 0 -> 4.7%. So no shape separates the populations at all (the claim's `enumeration` carries the whole table, shape by shape, each verdict written from its own two numbers). What is genuinely absent is a write for the recall itself: the local recall path refills the input box and slices the in-memory array without appending anything (TURN-025's traced hops). The consequence a consumer needs: a hook running at prompt submit cannot apply the label, since the superseding record IS the prompt being submitted; the closest approximation - skip a trailing turn-opener that no assistant record follows - hides 817 of 886 drafts (92.2%) and wrongly skips 74 of 2,595 genuine messages (2.9%), and is documented as a heuristic in SKILL's wrong-assumptions table, never as the label. 69 of 886 (7.8%) drew an assistant record before the resend, which is why even that window shape is not a rule.
> ⑤ **C-28, record-level delivery visibility (BREAKING for the bare-role sets).** A bare ROLE selector now selects what the model RECEIVED, decided per RECORD instead of per leaf (§5.5). The assembler that builds an API request runs ONE drop predicate, read whole in the shipped binary (claim CLS-026), and it disagrees with the leaf table in three cases: a `system`/`local_command` record - a slash command's own echo and its stdout - is RE-MINTED as a user message and sent, while an `isVirtual` record and the `<synthetic>` API-error placeholder are dropped. `Record::delivery_override` mirrors those three and nothing more; the predicate's other two arms (an `api_system` KEEP, a `progress` drop) need none, because csift models no leaf for either type. The seam is `collect_record_hits`, once per record, right after classify and before any label is tested, so the hits path and the label census cannot drift; `labels[]` stays classification and the verdict rides beside it as the JSON hit field `delivered`, with `[not delivered]` in the label zone under an explicit selector. Corpus effect, the two builds run back to back over 72 top-level transcripts: `-t harness` gains 16 records (the whole `local_command` population), `-t agent` loses exactly 118 `agent.message` records (the whole `isApiErrorMessage` population, 118 of 118 also carrying the sentinel model). Globs, intermediate prefixes and exact leaves are untouched. Two gate consequences ride with it: the key-only `"subtype"` needle is admitted under a bare `harness` role so the delivered record can be reached at all, and the §7f synthesized-marker set reads the SAME predicate, because a narrower one would prune a file whose only hit lies in the fabricated `[<subtype> <level>]` head. `gated_leaves_unreached` and its stderr note now report what the scan PARSED, and the note's leaf list is derived from `GATED_LEAVES` instead of a hard-coded string.
> ⑥ **What a slash command leaves on disk, and what of it the model sees** (claims TURN-027 through TURN-030, MISC-031). A local-jsx command decides what is WRITTEN through the `display` option its completion callback returns, and the executor has exactly four outcomes: `skip` writes nothing; `system` writes the command echo and a `<local-command-stdout>` body through the `system`/`local_command` factory; any other value writes an ordinary `type:"user"` wrapper plus a `<local-command-stdout>` user record; and a panel closed with ESCAPE writes nothing whatever the command's own option was. The `local` command type has no display axis and its result map is separate: a `text` result goes through the SAME `system`/`local_command` factory (choosing the `local-command-stderr` tag when the level is error), a `compact` result writes a user record, a `query` result writes the wrapper plus stdout plus an isMeta prompt, and a `skip` result writes nothing. So the 169 `<local-command-stdout>` USER records in the reference corpus come from the local-jsx default arm (login 69, model 75, effort 4, passes 1, config 1 = 150) and from `/compact`'s own compact arm (19), never from the `local` text arm. Three consequences a consumer needs. The absence of a wrapper is not evidence a command was not run: `/btw`, `/tasks` (= `/bashes`) and `/release-notes` return skip on every normal exit, any Escape-dismissed panel writes nothing, and `/rewind` (= `/checkpoint`/`/undo`) and `/stop` take the `local` skip result. A record on disk is not a record the model saw: `/release-notes` appends a `system`/`informational` notice and THEN returns skip, so it is rendered to the human and dropped by the assembler, like every harness notice. And the one delivered carrier has a single minting site - the `subtype:"local_command"` literal has exactly two occurrences in the executable, the factory and an SDK rehydration mapper - which is why csift keys the delivered case on the record shape alone and never on a command name or a tag.

> **v0.11.0 CHANGE LEDGER (BREAKING; THE CSIFT CHANNEL -- `csift 0.11.0`).**
> ① **The contract change, first, because everything else rests on it: csift's read-only law is amended.** Through 0.10.5 csift wrote nothing at all. From 0.11.0 there are exactly THREE writers - `send`, `deliver` and `ack` - and they write exactly ONE place: the csift-owned sidecar directory `<session-sidecar-dir>/csift-channel/` (§1.1), the `<uuid>/` directory beside `subagents/`. Never a transcript, never the team mailbox, never the cross-session messaging socket, never the session registry, never any settings file. csift does not install its own hook: `deliver --recipe` PRINTS the settings block and the user pastes it. csift never performs an official send either - where an official transport exists the receipt prints the exact call for the model to make, because those transports are model tools and csift is a binary. Every other subcommand stays read-only. This is why the release is a MINOR bump with the `!` marker: the promise a reader had ("it only reads") is narrower now, and a narrower promise is a breaking one even when nothing that used to work stops working.
> ② **Four new subcommands (13 -> 17), one message channel.** It exists because the official channel is STRUCTURALLY absent for some receivers, not because it is nicer: a running workflow lane cannot be reached by the send tool at all (its transcript lives under `workflows/<run>/`, a path the harness's own resume resolver never reaches), an unnamed subagent has no arm to address its parent subagent (the official `to` grammar resolves `main`, a name, a `[ref]` or an `a…` id, so addressing `main` reaches the top-level conversation instead of the spawning agent), nothing reaches an idle top-level session that published no socket, a sender OUTSIDE Claude Code holds no tool to call at all, and a build below the version floors or with the gates off has nothing. `send` (§6.15) queues one message for one resolved lane and prints ONE verdict (`OK` | `FULL` | `MAY-FAIL` | `UNPREDICTABLE` | `REFUSED`; exit 0 for every one of them, a refusal being a definitive answer rather than a usage error). `deliver` (§6.17) is the hook entry: the user pastes `csift deliver --slot k` lines on the eight delivery events, and slot k emits chunk k of what is waiting. `msg` (§6.16) joins the ledger's INTENT to the receiver transcript's FACT into one verdict (`DELIVERED` | `INTENT-ONLY` | `QUEUED` | `HELD` | `EXPIRED` | `ACKED` | `REFUSED`); `ack` appends the one line only the receiver can write. The design is thin-hook / fat-csift on purpose: the pasted line is identical on every event and carries no logic, while append-only accounting, dedupe by id, atomic appends, chunking, slot ordering and block-cap awareness all live in the binary, where they can be tested.
> ③ **`whoami` grew a lane layer** (§6.18): a `@<agent-id>` or `@<Name>@<Team>` target answers for THAT lane; three sections (`self`, `parent`, `topology`) follow the identity output; `--to <TARGET>` runs the same policy table as a prediction and writes nothing; `--peers` lists every live lane as `id kind state` and NOTHING else - no description, no agent type, no name read as a role, because that is exactly the material one lane would use to claim standing over another. Outside Claude Code, `whoami` prints the not-a-lane answer (the one channel out, and what a receiver needs installed) on stdout and still exits non-zero, because the identity question has no answer there and guessing one is the documented trap.
> ④ **The 35th leaf, `agent.communication.channel`** (§5.1). A delivery lands in the receiver's transcript as a `hook_additional_context` attachment; the leaf classifies it when the FIRST content string opens with `[csift-channel v1 `, keeps `harness.meta.hook` beside it and LEADS it. It is the one attachment leaf a DEFAULT scan reaches - every other attachment keep is flag-gated, and a message addressed at this lane is not machinery. Because the keep admits any LINE carrying the literal, an ordinary attachment that merely MENTIONS the header (a quoting hook context, an `edited_text_file` snippet) is parsed too and is dropped again before it can surface: on a flagless scan with no address, a record whose only views are the two gated attachment leaves is not kept. The render is verbatim (the header line plus the body chunk), so no synthesized-marker machinery is needed and the §7d/§7f gates hold unchanged; the candidate needle `csift-channel v1` is a bare value substring with no JSON-escaped character, `&&`-gated on the label selector, so a `-t user` / `-t harness` query pays nothing. The envelope's own fields live in the CONTENT array rather than in the hook name, because a hook attachment's `hookName`/`hookEvent` are not part of its searchable text - a channel whose id and sender lived there would be unauditable by any default query, csift's own included.
> ⑤ **The settings cascade as a shared model** (§1.2, `path::settings`). Five scopes folded in Claude Code's order, `env` per key and `hooks` concatenated per event, the composed first-wins policy tier, plugin hook manifests unioned at read time, the three policy switches evaluated after the fold, and a named UNOBSERVABLE list (`--settings`, `--managed-settings`, `--setting-sources`/`--restricted`, the trust-dialog state, MDM) so a gate verdict says "unknown" with its evidence instead of implying the file scopes are all there is. `plan`'s `plansDirectory` lookup moved onto it with its semantics unchanged: the same three file scopes in the same precedence, so a git-root local file that the general model reads does not enter the plan binding.
> ⑥ **The teammate ROUTING form `@<Name>@<Team>`** (§2, §6.5). A teammate carries two ids, minted apart at spawn: the routing form is what the official `SendMessage` addresses, the transcript form `a<Name>-<16 hex>` is what its hook payload and its `agent-<id>.jsonl` stem carry. Only the routing form can COLLIDE (two same-named teammates in one team share it), so csift keys targeting, its lane files and the envelope's `to` on the transcript form, resolves the routing form through the teammate metas (`name` and `teamName` matched exactly, no case folding) and fails loud on an ambiguity by listing every matching transcript id. Every surface that prints a teammate now prints both forms: the `agents` node line gains `routing: <Name>@<Team>` and its JSON a `routing_id`, and the teammate control hint gains the sentence that names which tool needs which id.

> **v0.10.5 CHANGE LEDGER (non-breaking; the ledger correction round -- `csift 0.10.5`).**
> ① Four defects the ledger's own `drifted` verdicts had named and 0.10.3/0.10.4 shipped around, each fixed with its claim: (a) `image` joined `#N` by a positional zip of the `[Image #N]` markers against the image blocks; the record's `imagePasteIds` lists the ids in BLOCK order (the submit path writes the blocks and the array in one ascending-id pass) while the markers keep the operator's text order, and the two diverge on 18 of 662 corpus records (ids always ascending, the same set, two or three swapped in the text), so those got a silently wrong handle. The join is now by number through `imagePasteIds` (block k takes id k), with the positional zip only as the fallback for records without the array, under its count guard; a `queued_command` attachment's prompt takes the same join - IMG-005. (b) `recover` replayed a Read echo with no text: the harness blanks the text of a tool result older than its retention window before persisting it (`content` empty, `numLines`/`totalLines` intact; 320 in the reference corpus) and four result arms (`file_unchanged`, `pdf`, `parts`, `notebook`) never carry one; a blanked echo of a complete read satisfied the whole-file test and replayed as a snapshot of nothing. Such an echo is now `EventKind::BlankedRead`: counted (`blanked-read` in the coverage line, `events.blanked_read` in JSON), disclosed as a soft authoritative boundary (`blanked_read`), never spliced; a genuinely empty file stays an ordinary read - FH-048. (c) The `status`/`wait` pid probe ran `ps -o lstart=` with the inherited locale and parsed two English field orders in the local zone; a German or French `LC_TIME` renders non-English names (`Fr  4 Sep`, `Ven  4 sep`) and the pid-reuse guard was silently skipped. The probe pins `LC_ALL=C` and `TZ=UTC` on its own call, exactly as the harness pins its `procStart` acquisition, and parses as UTC - MISC-024. (d) A line number is a durable address only while the transcript is append-only, and three harness paths rewrite a live transcript in place (a stream tombstone truncates and rewrites the tail; the local compaction rewrite; a remote-ingress resume), shifting the lines above the cut so a kept `--line` pointer resolves silently to another record. Every search hit now carries `refetch_uuid` (`csift show @<id> --uuid <uuid>`) beside `refetch`; the search help states the durability rule - REC-093.
> ② Four statements csift printed or taught were wrong at 2.1.258 and are corrected with their dating: TaskStop resolves a teammate by its name or `name@team` from Claude Code 2.1.198 (the teammate-aware resolver is absent in 2.1.197 and present in 2.1.198, dated against the cached native builds); the agents footer, the JSON `control_hint`, the SKILL rows and hook recipe, AGENTS 3.7 and section 6.5 said it rejects every form, which was true on the build of the original incident - SUB-020. The file-history checkpoint store is written by the structured file tools AND by an approved in-place `sed` preview (the shell tool applies a previewed `sed -i` through the checkpoint path instead of running it); `recover --list-backups`, its help and the module doc said bash edits never land there - FH-024. The pending AskUserQuestion split (a multi-question ask on disk while its dialog is open, a single-question ask buffered) is a timing outcome of the persistence frontier, which has no question-count branch and did not change at 2.1.258; the verdict comment, the tail note, the live help, AGENTS 3.4, section 4 and 6.13 stated a question-count rule since 2.1.258 - ELI-009/ELI-012. The registry row writer stamps `procStart` unconditionally on every platform; the two-key `procStartFt` schema belongs to the `<pid>.<hash>.key` companion, and the reader's comment attributed it to the row - WIN-007.
> ③ Ledger schema and gate: the attribution `upstream` (`producer_trace: upstream` + `specimen: observed`: the producer lies outside the shipped binary by construction - the model or API side, the operating system, a native runtime binding - while the client-side treatment is traced; a failed search is never upstream); gate rule 9 (a claim's latest check is never `drifted`: the fix or the retirement appends its own check), rule 10 (no open leg is an unconsumed `TEXT` correction or a recorded rejected rewrite: both mean a refuted text still ships), rule 11 (every claim below end-to-end carries both leg fields); the README tally gains the `upstream` row. The prose law for ledger text: no calendar dates, no csift versions, no audit narrative in any text field; the commit and the release carry the when and the which. The README verification section states what the project will never publish (the method, a mechanical ledger comparison, any corpus) and why the ledger is re-verified periodically rather than per Claude Code release.
>
...
> ③ **Two HITL instruments beside the sidecar.** Registry `status:"waiting"` => `waiting-hitl` (the binary's `waitingFor` kinds: a queued elicitation, any top dialog including permission prompts and plan approvals, a worker request, a sandbox request); an unreturned `AskUserQuestion`/`ExitPlanMode` at the main tail => `waiting-hitl`, because a MULTI-question ask is written at question time since 2.1.258 while a single-question ask stays buffered (section 4; measured three single-question and two multi-question trials on macOS and Windows) [v0.10.5 correction: the measurements stand, the causation does not - the split is a timing outcome of the persistence frontier, which has no question-count branch and did not change at 2.1.258; section 4 and the v0.10.5 ledger state it as measured]. The idle-verdict honesty note now names the registry status it saw. `verdict::assess_full` split into `collect_evidence` + `rank_verdict` (the complexity gate).
...
**Clear lineage (v0.12.2).** The OTHER way one session succeeds another, and the harness writes no field for this one at all (§4.10). A top-level transcript is `minted_by: "clear"` when its FIRST non-isMeta user record is the `/clear` slash wrapper. The wrapper for the command that ended the previous session lands in the file that command created, so the new file is self-identifying (the isMeta `<local-command-caveat>` record may precede it and is skipped; a `/clear` wrapper anywhere BELOW the first user record belongs to the clear that LEFT this file). The predecessor join is the cost ledger's checkpoint: a sibling in the same project dir carrying a `cost-state` line whose `startTime + totalDuration` falls within 2000 ms of the wrapper's timestamp. The nearest LINE wins (a file can carry several checkpoints); a tie between two different FILES is reported and joined to neither, the same fail-loud posture the clone join takes. Measured on a live clear: 164 ms, against a nearest competitor 268 times further away. The row reads `cleared  from <first8> (an inference: the checkpoint the clear wrote closes N ms before|after this file opens)`, and an unjoined mint still reports itself (`minted by /clear; no cost-ledger checkpoint within 2000 ms in this project dir`). JSON: `minted_by` (`"clear"` | null), `cleared_from` (null on a tie or a miss), `cleared_from_distance_ms`, `cleared_from_candidates` (the tied ids). Cost: the mint probe rides the head scan the row already runs, and the sibling sweep (a memmem for the `cost-state` literal, parsing only the lines that carry it) runs ONLY for a transcript that opens with the wrapper.

**Handoff lineage (v0.12.3, claim REC-106).** The THIRD way one session succeeds another, and the only one Claude Code states outright. When a session is handed to a BACKGROUND child (the left-arrow gesture, or the background-fork command), the harness appends one line to the PARENT transcript: `{type:"continued-in", timestamp, sessionId, continuedInSessionId}` — four keys, no `uuid`, no `message{}`, no `role`, so the line is not a chain node, the loader never admits it to the message array, and its only address is its jsonl line. `list` reports the child id verbatim: the row reads `handoff  continued in <first8> (a background handoff: Claude Code names the child itself, so this one is no inference)`, and JSON carries `continued_in` (the full id, null when absent). Two facts bound what the field means. An in-place `/fork` reaches the same spawner with `keepParent` and writes no such line, so an absent field never means "not forked". And the append also runs on the FAILED-spawn path once a rescue row is queued, so the field does not prove the child ran — Claude Code's own reader opens the named child transcript and requires conversation content in it before treating the parent as superseded, and the corpus specimen's child holds 2 bookkeeping lines and no conversation. WINDOW LIMIT, stated because `list` is a head/tail reader: the line is appended after the last conversation record, so the backward tail walk normally reaches it before the anchors that stop the walk. Precisely, the tail read stops once it holds BOTH anchors - the last genuine-user and the last agent message - so a later `--resume` of the parent that appends both of those below the line pushes it outside the window and `continued_in` reads null; a transcript missing one of the two anchors is walked to the top of the file and loses nothing. Two whole-file answers exist: `list --lineage` (v0.12.3, the §6.1 lane-lineage paragraph) and `stats`' line-type census, where `continued-in` is already one of the `types` keys. Cost: the line carries no `role` marker, so `line_is_list_candidate` gains one `||`-arm memmem on the quoted KEY `"continuedInSessionId"` — the R13 key-only needle form, which survives a reserialize and which a prose mention of the type value cannot trip; a quoted key inside a payload is not a top-level field, so serde reads nothing from it.

**Lane lineage (v0.12.3, claim REC-107).** Not a succession link but a LANE fact, and it answers "was this written by a background process?". Every transcript record a background lane writes carries a top-level `sessionKind:"bg"`. A foreground lane's records carry no such key at all: one stamp builds the value from one environment variable through a three-value allowlist (`bg` | `daemon` | `daemon-worker`), the value is undefined in a foreground process, and the serializer drops an undefined property. Two consequences shape the surface. It is a per-PROCESS fact, so `session_kind` is an ARRAY and is EMPTY for an ordinary session rather than carrying a sentinel - and a single transcript can hold a background lane's records and, below them, records a foreground resume of that same session appended without the key. And the values come from the head/tail windows, which is where Claude Code's own session summary reads them too (it lifts the key off the FIRST record of the file), while the SPAN cannot: the stamp stops mid-file and no window can see where. Hence `--lineage`, ONE whole-file pass that fills `session_kind_first_line` / `session_kind_last_line` (text: `lane     bg on L<a>..L<b>`) and sets `lineage_scanned`, so a consumer can tell "not asked for" from "no carrier"; without the flag both lines are null and the row reads `lane     bg (seen in the head/tail windows; span needs --lineage)`. The pass is `parse::lineage_fields`, a depth-1 key walk of the shape the chain spine already uses: it skips every value with the SIMD string-jumping walk and decodes only the two spans that matter, so a megabyte attachment line costs a key walk instead of a serde decode. Two properties follow from that and both are load-bearing. A key NESTED in a payload is not a field, so a transcript quoting the shape reports nothing - the same answer the record model gives. And there is no parse to FAIL, so the pass adds NO malformed count: `skipped_lines` keeps its R12 meaning (a census of the head/tail lines read, each booked once) instead of gaining a third overlapping source, and `stats` stays the whole-file census authority. The same pass resolves `continued_in` over the whole file, so `--lineage` lifts the handoff window limit above as well. Neither needle can be tripped by prose: a `"` inside a JSON string is escaped, so a quoted key mentioned in a message body is the bytes `\"sessionKind\"` and not the needle.
...
| `--lineage` | flag | off | answer the session-lineage fields over the WHOLE file instead of the head/tail windows: the `sessionKind` SPAN (`session_kind_first_line` / `session_kind_last_line`, null without it) and an exact `continued_in`. ONE depth-1 key walk per file, no record parse, so it adds nothing to `skipped_lines`; measured 2.1x the same-binary control band on a 720 MB transcript, which is why it is opt-in |
...
**A "turn" is delimited by a boundary record** — `opens_turn(record)` (§4.1): a genuine human message, an answered AskUserQuestion (§4.4), a tool-use rejection-with-message (§4.2.4), or an inbound peer message in any of its three framings. A non-boundary `tool_result`-carrier, an `isMeta` pseudo-turn, an interrupt / `<local-command-stdout>` / `<command-name>` synthesized marker (§4.2.1–.3), and a compaction summary never start a turn. Turn index is 0-based in boundary order within a session, and only LIVE turns are numbered (§6.4.1).
...
### 6.4.1 LIVE turn numbering: the survival axis applied (v0.12.0)

**A turn index counts only turns the surviving conversation still reaches.** §4.11 defines the axis; this is the numbering rule that falls out of it, and it is the SAME rule on every command, so a `verbatim` turn 7, a `search --count-by turn` row `t7`, a `show --turn 7`, a `files --turn 7` and a `recover --at @turn:7` all name one turn.

- **Delimiting.** `model::group_turn_indices_chained(records, rec, &chain)` is the one production delimiter: an ABANDONED record belongs to no numbered turn, neither as a boundary nor as a member. The bare `group_turn_indices` is the test-only boolean base, and `group_turn_indices_deduped` (which trusts a hole-punched DAG only as far as the superseded opener) has no production caller and stays as the documented middle rung.
- **TWO ways in, one answer.** `search`/`show`/`stats` take their scan's two output streams, the records and the structural spine rows, and merge them back by line into ONE borrowed file-order row list (`search::Row`, v0.12.1 - the index space every chain answer is addressed by), then call the chained grouper with a projector into it. The file-oriented commands (`verbatim`/`files`/`recover`/`image`) index everything by their own record POSITIONS and cannot splice a foreign row into that list, so they go through `model::ChainView`: `parse::parse_candidates_with_spine` lifts a dropped line's spine row in the SAME pass, the view merges the surface's records with that `&[SpineRow]` slice by line number, runs the ONE chain over the merged DAG, and answers back in the surface's own index space: `survival(i)`, `stamp(i)` (`TurnStamp::{Live(n), Abandoned{root_line}}`, whose `text()` is the ONE spelling of a turn slot: `7`, or `abandoned (root L42)`), `order_turn(i)`, `turns()` and `groups()` (the live turns THEN one group per abandoned branch, so a per-turn extractor keeps each branch's own `tool_use`/`tool_result` join scope). Both ways answer identically on the same DAG.
- **Windows.** `--turn` and `--since`/`--until` resolve against the live numbering. An abandoned record carries no turn index to window on, so the two families of command answer the question differently, and the split is not a style choice.

**THE DISK / CONVERSATION SPLIT.** What a surface does with an abandoned record depends on one question: is this command reporting the CONVERSATION or the DISK?

| | reporting | abandoned record | under `--turn` |
|---|---|---|---|
| `search` · `show` · `stats` · `verbatim` · `image` | the conversation | outside turn numbering; `verbatim` counts it in a footer with the `csift show --line` that fetches it and never replays it (claim CMP-020); `search`/`show` reach it by a glob or an exact leaf with a marker; `image` still LISTS and extracts it, marked | suppressed |
| `files` · `recover` | the disk | the row and the event are KEPT and MARKED (`[abandoned]`, a turn slot reading `abandoned (root L<n>)`, JSON null `turn_index` beside `survival` and `abandoned_root_line`); the replay stays FILE ORDER and the rollups add the abandoned SHARE without subtracting it | ADMITTED, keyed on the live turn the row physically FOLLOWS (`ChainView::order_turn`) |

An Edit made on a branch the operator later rewound past LANDED on disk, so a `files` report that hid it would be wrong about the filesystem, and a `--turn` window over a span of the transcript must not silently omit a write that happened inside that span. A compaction summariser reads the in-memory message array, so it never saw an abandoned turn either, and replaying one into a "what the compaction clipped" reconstruction would put text there that no model read. The two families make opposite calls for the SAME reason.
...
**Kind is the on-disk PATH LOCATION, not `agentType` — with ONE meta-driven exception, `teammate`.** Both `builtin-task` and `workflow` carry the same spread of `agentType` values (`Explore`, `general-purpose`, `oh-my-claudecode:*`); only `workflow-subagent` is workflow-exclusive. So `agentType` is a descriptive sub-label, not the discriminator. A **teammate** (Claude Code's `in_process_teammate` / FleetView feature) sits at the SAME location as `builtin-task`, so location can't classify it — the meta `taskKind:"in_process_teammate"` upgrades it to `teammate`. Its id EMBEDS the teammate name (`aVSRepro-68a2a1661c9390c1` = `a<Name>-<16hex>`, NOT a bare hex), its meta overloads `agentType` with the handle and omits `toolUseId`, and it is spawned by an `Agent` tool_use whose `input.name`==the handle + `input.subagent_type`==the REAL type. csift recovers the real `agent_type`, spawn `tool_use_id`, true trigger ts, and parent via a NAME-join (`meta.name` ⇆ `Agent.input.name`), and surfaces `name`/`team_name`. **`agents` also emits a CONTROL pointer** (csift never drives the harness's own control tools, but it names the one that can): a text footer when ≥1 teammate is in scope + each teammate node's JSON `control_hint` — a teammate is steered/terminated via `SendMessage` BY NAME (`message:{type:"shutdown_request"}`); from Claude Code 2.1.198 `TaskStop` also resolves a teammate by its name or `name@team` (earlier builds reject every form, and the `aName-<hex>` transcript id is never a task id on any build); never `pkill` (in-process; shares the orchestrator PID). **The canonical agent id** (bare `<hex>` for built-in/workflow, `a<Name>-<hex>` for a teammate) is what every surface prints AND re-accepts as `@<id>` (`path::is_subagent_id`), not the `agent-…` filename stem. **Since v0.11.0 a teammate node prints BOTH of its ids**: the text node line carries `routing: <Name>@<Team>` beside the transcript id and the JSON row carries `routing_id` (null when the meta gave only one half — a teammate without a `teamName` has no routing form, and csift does not fabricate one). The control hint gained its third line for the same reason: the official `SendMessage` needs the routing id, the hook payload and the transcript file carry `a<Name>-<hex>`, the routing id CAN collide and the transcript id never does. Both forms target csift itself (§2), and the routing form resolves through the same metas.
...
**Per-subagent (`kind:"agent"`) node fields emitted:** `agent_id` (bare hex, or a teammate `aName-<hex>` id — the name may itself carry dashes), `shape` (the transcript shape — `builtin-task`\|`workflow`\|`teammate`; JSON key is `shape`, not `kind` — `kind` is the envelope's row tag), `parent_session_id`, `parent_agent_id` (the spawning agent for a nested subagent; `null` ⇒ a direct child of the session, depth 0), `spawn_tool_use_id`, `spawn_tool` (`Agent`/`Task`/`Workflow`), `workflow_id` (workflow only), `agent_type` (meta `agentType`, fallback the spawn's `subagent_type`), `name`/`team_name` (teammate only, else null), `description` (built-in meta, fallback the spawn's), `trigger_utc`/`trigger_local` (the true spawn instant), `started_utc`/`started_local` (first transcript record ts), `completed_utc`/`completed_local` (the completion instant — non-null ONLY when `status` is `completed`; a frozen/running/unknown lane carries `null`), `last_activity_utc`/`last_activity_local` (the tail newest-record instant, present on every timestamped lane regardless of status; == `pending_since_utc` on a frozen lane), `duration` (trigger→completion; null unless completed), `status`, the FROZEN-LANE fields `pending_tool_use_id`/`pending_tool_name`/`pending_classification`/`pending_reason`/`pending_checker`/`pending_since_utc`(+`_local`) (all `null` on a normal lane), `depth`, `skipped_lines`, `control_hint` (teammate only); plus on demand `returned_message` + `returned_message_source` and `files_changed[]`. **Frozen-lane detection:** when a subagent's NEWEST meaningful record is an UNRETURNED `tool_use` (no following `tool_result` for its id — a permission escalation leaves NO jsonl trace, the lane just freezes there), `status` is forced `running` (NEVER `completed` — overriding the end-of-turn-text walk-back, which would otherwise read the assistant text PRECEDING the frozen tool_use as a clean finish) and `pending_classification` ∈ {`escalation-blocked`, `awaiting-execution`}: `escalation-blocked` iff the harness would HOIST the pending Bash command for human approval even under bypass (≈ waiting for a Yes); else `awaiting-execution` (slow tool OR wedged — indistinguishable in jsonl; weigh elapsed-since-`pending_since_utc`). **v0.12.2** makes the prediction generational and says how it was reached. CC decides a removal with ONE of TWO checkers and the DECOMPOSITION picks which: a `too-complex` parse reaches the lexical classifier, a clean parse the structured checker, and both write the same bypass-immune ask — so both can yield `escalation-blocked`. `pending_reason` carries the harness's own reason tail verbatim for a predicted ask (`on critical path: /tmp`, `on possibly-empty variable path: "$S/$f"`, `on statically-unresolvable target: …`, the census bail's `— too many command substitutions to analyze (N)`), or csift's note `removal target needs the filesystem state at the time` when the deciding arm resolves paths against the shell cwd and the workspace set, which a transcript does not carry — that verdict stays `awaiting-execution` and is never guessed either way. `pending_checker` is one disclosure line, `path: <structured|lexical (…)> · checker: <structured|lexical|census> · <gen1|gen2|gen3>[ assumed]`, where the generation comes from the record's own `version` (`< 2.1.208`, `2.1.208..=2.1.260`, `>= 2.1.261`) and `assumed` marks a record that carries none. Both fields are `null` for a pending call these lexical layers do not model. A pending **`PowerShell`** lane (Windows' separate shell tool, v0.7.4) always classifies `awaiting-execution` with a null reason: the lexical-bash layers deliberately do not run on PowerShell commands.
...
**The survival axis (v0.12.0) — disk truth is FILE ORDER.** Turn numbers here are the LIVE ones (§6.4.1: the turns Claude Code's own conversation chain still reaches, the same numbers `search`/`show` print). A mutation made under a prompt the operator later rewound past, or under one recalled and re-typed, belongs to no numbered turn: the turn slot reads `abandoned (root L<n>)` naming the branch head and the row carries an `[abandoned]` marker (`[pre-cut]` above a compaction cut). **The row is never dropped.** That write really did land on disk, and a `files` report exists to answer "did this session write X?" — a survival-filtered answer would be wrong about the filesystem. The rollups keep the two figures apart instead: `ops_label` appends `(N from abandoned branches)` and every grouped JSON row carries `abandoned` as a SHARE of `total`, never a subtraction. Consistently, a `--turn` window ADMITS an abandoned row by the live turn it physically follows (its ordering key), because a window over a span of the transcript must not silently omit a write that happened inside that span. The conversation-oriented surfaces make the opposite call for the same reason — `search`/`show`/`image` suppress abandoned units under a `--turn` window, since they are answering about the conversation, not the disk.

**Output.** Text groups under a `SESSION <id>` header, then the level-appropriate body, then the Edit-before-Read boundary section (if any), then a footer: distinct files + total mutations + boundary count, the active detail level (`detail=…`), the turn/time filter context (`turn=<SPEC>` when `--turn` is set), the Bash-heuristic caveat, and the skipped-line count. Empty result (no mutations AND no boundaries) prints `no file mutations found`. JSON is the §8.2 envelope: the `{kind:"header",…}` line, then one object per emitted unit (bucket / dir / file with the counts + first/last; or per mutation for `--by timeline` with `{kind:"mutation", session_id, is_subagent, parent_session_id, path, op, ts_utc, ts_local, turn_index, survival, abandoned_root_line, line, is_create, heuristic, resolution, path_verbatim, command_errored}` — `resolution`/`path_verbatim` are the v0.8.0 bash-resolution provenance, null on structured tools and class markers; `survival` is `live`/`pre-cut`/`abandoned` and an abandoned row has a NULL `turn_index` beside its `abandoned_root_line`, v0.12.0), then one `{kind:"boundary", session_id, is_subagent, parent_session_id, path, line, turn_index, survival, abandoned_root_line, cause, ts_utc, ts_local}` per boundary (in every detail mode; `cause` names what changed the file out of band — `kind` stays the envelope discriminator exclusively), then the trailing `{kind:"summary", sessions, distinct_files, total_mutations, edit_before_read_boundaries, skipped_lines, detail_level}` (`sessions` = the distinct owning sessions among emitted rows — v0.5). `files` has NO row cap, so the summary deliberately carries NO `dropped_by_cap`.
...
| blanked Read echo (v0.10.5) | a Read `file` object with NO text: `content` absent (the `file_unchanged`, `pdf`, `parts`, `notebook` result arms) or blanked to the empty string while `numLines`/`totalLines` still count lines (the harness clears the text of a tool result older than its retention window before persisting it; 320 in the reference corpus) | a counted `BlankedRead` event (`blanked-read` in the coverage line, `events.blanked_read` in JSON) with an AUTHORITATIVE non-invalidating ANNOTATION boundary (`blanked_read`); never spliced, never a snapshot — a blanked echo of a complete read used to replay as a whole-file snapshot of nothing. A genuinely empty file (`totalLines` 0) stays an ordinary read |
...
| `file-history-snapshot` / `file-history-delta` | `snapshot.trackedFileBackups[<path>]`, and (v0.12.0) the delta line's `trackingPath` + `backup` — the per-write form of the same instrument, admitted by the shared `file-history-` type prefix | a coverage ANNOTATION, plus the version SEQUENCE the external-write inference below reads. `backupFileName` is usually present (measured 83–98%), but the store it names is PRUNED and checkpoint content has no transcript anchor, so it is **never** used to fabricate content (`--list-backups` lists the store) |
...
**Integrity boundaries (a point where reconstruction across it is invalid), in confidence order:** (1) AUTHORITATIVE — a `modified since read` harness error; (2) AUTHORITATIVE — Claude Code's own `staleReadFileStateHint` naming the file (`hint_modified`); (3) AUTHORITATIVE — an Edit whose `originalFile` disagrees with the replayed buffer (the signal claude-file-recovery *discards*, csift mines — a soft bash boundary does NOT disarm this check, v0.8.0: catching post-bash drift is its job); (4) AUTHORITATIVE — an external `edited_text_file`; (5) AUTHORITATIVE annotation — `stale_recovered` (nothing invalidated); (6) HEURISTIC — a Bash mutation (always flagged best-effort, naming the resolved path + class). **The HARD/soft split (v0.8.0):** hard = the INVALIDATING kinds (`modified_since_read`, `external_edit`, `hint_modified`); soft = annotations + heuristics (`original_file_disagreement`, `stale_recovered`, `blanked_read`, `bash_mutation`). `fragments = hard + 1`, so soft signals never inflate it; coverage prints `N (h hard · s soft)` and JSON carries `hard_boundaries`/`soft_boundaries`. `--patches` emits one unified diff per segment between segment-splitting boundaries, each segment + boundary carrying its jsonl line / turn / timestamp; **no diff spans a boundary**. Boundary rows also carry `source_session_id` + `source_line` — the REAL transcript location — while `line` stays the replay/cutoff coordinate (`--at @line:<N>`); the two differ only after a cross-transcript merge. **Final-state invalidation (load-bearing for honesty):** a `modified since read` boundary means the file changed out from under the harness (a `prettier`/linter rewrite, etc.) and a fresh Read was demanded — so every line known BEFORE that boundary is now SUSPECT. The final-state reconstruction therefore INVALIDATES the pre-boundary buffer (clears the known lines + the seen-total); only content RE-READ / re-written after the boundary survives into the final state. Pre-boundary lines become explicit gaps, never silently-stale lines presented as "current". This is what stops restore from falsely reporting `complete` (and `--salvage`/`--at @latest` from dumping stale content) when a session read a file, watched it change, and only re-read part of it.

**The external-write row (v0.12.0) — the C-24 instrument, scoped to the target.** Claude Code bumps a tracked file's `version` only when its bytes changed, so a version JUMP with no replayed write of that file since the previous marker is a write with no tool record. `recover` reports it for the `--file` target on ANY path, as an AUTHORITATIVE `external_write` boundary whose detail names the interval it bounds: `external write (inferred, snapshot vN->vM, no tool record since L<line>)`. `files` (§6.6) reports the same inference only for the settings family, and the difference is a scope law, not an inconsistency: the tracked set spans every written path, so a global timeline row would flood the output (censused corpus-wide: 58,115 such jumps over 1,121 non-settings paths in 31 of 75 top-level transcripts, against 47 in the settings family), while a row about the ONE file a caller asked to reconstruct is exactly the fact they came for. Both file-history line shapes feed the sequence, so the interval is as tight as the harness's own bookkeeping allows. The motivating producer is the /rewind **Restore code** gesture, which writes NOTHING to the transcript — the next prompt's `edited_text_file` attachment remains the authoritative boundary it already was, and this inference is what covers the gap before it.

**The survival axis (v0.12.0) — the replay is FILE ORDER and stays so.** An Edit made on a branch the operator later rewound past DID hit the disk, so it is extracted and replayed like any other write; the abandoned branches are extraction GROUPS beside the live turns (§6.4.1), which also keeps each branch's own `tool_use`↔`tool_result` join scope intact. What the axis changes is the REPORTING. Turn numbers are the live ones — the numbering `search`/`show` print, and the domain `--turn` and `--at @turn:` resolve against — while an event or boundary from a record off the surviving conversation is stamped `turn abandoned (root L<n>)` instead of a number (JSON: null `turn_index`, plus `survival` and `abandoned_root_line`), and a one-line disclosure names how many such events the window held (`abandoned_events` in the coverage/snapshot JSON). A `--turn` window still ADMITS them, keyed on the live turn they physically follow: a window that silently dropped a real write would reconstruct a state the disk never had.
...
**A `/clear` ends the binding (v0.12.2, claim PLAN-021).** The session-id regeneration a clear runs calls `forgetPlanSlug` on the OUTGOING id before it mints the new one, so neither binding law can reach the predecessor's plan: a clear-minted transcript carries no `plan_mode` attachment and no slug of its own until plan mode is entered again, and `plan` reports none. That is the correct answer, not a gap (the new session genuinely has no plan), and it is why `plan` needs no clear-specific code: both laws already key on the TRANSCRIPT's own records, and the cleared file has neither. `list`'s `cleared_from` names the predecessor for a reader who wants the old session's plan; `csift plan @<that id>` is then the ordinary lookup.
...
**`--audit` (v0.12.3): four facts qualifying each binding** (claim PLAN-022; PLAN-002, PLAN-005, PLAN-006, PLAN-011, PLAN-014 and PLAN-016 are what they rest on). The audit already joined the scope's structured plan-file mutations against the corpus's bindings. These four say how much each binding in scope is WORTH, and each reads a fact on disk instead of deriving one.

- **(a) the slug's CHANGE POINTS.** `slug     L<n>  none -> <value>  (the slug is minted here)` for the mint; `slug     L<n>  <a> -> <b>  (the binding MOVED here)` for a value that changed. JSON: `slug_changes:[{line, from, to}]` with `from:null` for the mint, plus `first_slug_line`, `first_slug_utc` and `first_slug_local`. The binding csift reports is the HARNESS's, and this says when it became so - which is the question a fork raises, because a fork strips the slug from the records it copies and a first own compaction can mint a fresh one. Measured over every transcript of one projects root, 7,810 of 9,601 carry a slug and every one of those has exactly one distinct value and exactly one absent-to-value transition (0 value-to-value, 0 value-to-absent). A transcript printing two change points is therefore a shape the corpus has never shown; printing them is how a reader would ever learn of it. A transcript whose records carry no slug at all says exactly that rather than reporting an empty list as a value.
- **(b) whether the bound plan file is on DISK.** `[exists]` / `[missing]` beside the path, JSON `plan_exists` - the SAME `PlanRef.plan_exists` field the forward view prints (`Path::is_file` at resolution time), so the two views cannot disagree. `[missing]` is an ORDINARY state, not a fault: the plan name is minted at Plan-Mode entry and the `.md` lands only when content is first written (PLAN-014), and a deleted plan is still recoverable through `recover --file @plan`.
- **(c) plan text held with NO binding.** A `plan_file_reference` attachment - the post-compaction re-injection of the bound plan's whole content (PLAN-016) - present while NO record of the transcript carries a slug. Only the BOUND plan comes back in full, so this is text that is not coming back: a `warning:` line naming the carrier lines, a `plan-unbound-text` JSON row, and a count in `unbound_plan_text` and in `warnings`. It is reported for a transcript with no binding at all, which is exactly the state it describes. ZERO corpus specimens (all 14 `plan_file_reference` transcripts carry a slug); kept because the fork path that strips a slug can produce it. The detector reads the attachment's own `type`, so a prose mention of the type never fires it - the same rule the `plan_mode` binding follows.
- **(d) the slug against the plan file's BIRTH instant.** `birth    the first slug-carrying record is before|after|same the plan file's birth instant`, JSON `slug_vs_plan_file` over the closed set `before|after|same|unknown`. `before` is the ordinary order (the name precedes the file); `after` means this transcript bound a file that ALREADY existed, the shape a fork or a re-entry into an inherited plan takes. `unknown` is a real answer and names its reason in `slug_vs_plan_file_reason` - the file is not on disk, no slug-carrying record has a timestamp, or the platform records no file birth time (`std::fs::Metadata::created` errors there) - rather than picking a direction.

Cost: ONE walk per in-scope transcript, beside the binding resolution the audit already runs. The slug half goes through the depth-1 key walk (`parse::lineage_fields`) rather than a record parse, because a slugged transcript carries the key on MOST of its records and decoding each of those payloads to read one short string would cost the whole file; a key nested in a payload is therefore not a slug. The `plan_file_reference` half parses only the rare lines carrying that literal, because the attachment's own `type` is what decides it.
...
**Multi-agent-message richness (`--agent-msgs`).** A single user turn can own a LONG run of agent messages (a debugging/build chain the model narrates step by step) that the summary clips to its single §9 quote. Each `TurnSlice` carries EVERY agent-text record (`agents: Vec<AgentMsg>`); a derived `assistant_eot()` (== `agents.last()`) keeps the EOT anchor for dedup/round-trip/render. `--agent-msgs` decides how much of the run to restore — four modes: **`longest`** (DEFAULT) keeps the LONGEST agent message (the best one-message proxy for "where the substance is" — the summary's single quote is the turn's LAST message, often a ~50-char throwaway wrap-up, while the real finding sits in a MIDDLE one) **+** the first-when-substantive **+** every rich middle, collapsing the rest (including a short non-rich last) into a placeholder; **`eot-only`** forces last-message-only — byte-identical to the pre-expansion single-EOT output (the escape hatch); **`rich`** keeps the last always + the first by position privilege + each non-droppable middle; **`all`** keeps every message. `longest` applies to every multi-message turn; `rich` only filters a LONG run (`agents.len() > --agent-run-threshold`, default 6). A message is **kept when "rich"** — a cheap single-pass OR of a length gate (`>= --agent-rich-min-chars`, default 280) and a signal test (a number-of-substance, a commit-hash-like hex, a `file.rs:NNN`/`src/…` ref, a backtick code span, or a finding/decision lexeme). **KEEP-ON-DOUBT** is the spine: only a short (`< --agent-declaration-max-chars`, default 200) signal-less intent-verb opener (`let me …`/`now i …`) is collapsed; anything uncertain is kept (a wrongly-kept declaration costs ≤ one capped body; a wrongly-dropped finding is unrecoverable). A FUSED finding+declaration body trips a signal → kept WHOLE, its trailing declaration shed only by the char-ellipsis. A contiguous collapsed run renders as one `△ L{first}–L{last}  [X agent message(s) collapsed, N chars, Y tool call(s)[, Z failed]]` placeholder carrying the fetchable line range + per-message attribution + the summed chars it stood for, followed by one indented `    L{n}  {first 60 chars}… (+K chars)` preview line per collapsed message of at least 210 chars (v0.12.3 — see the ledger entry). `--keep-first` (default) keeps a turn's first message by position privilege **in `rich` mode** (`--no-keep-first` decides it as a middle); it has NO effect in `longest`, where the first is gated on length instead. `--profile heavy|light` bundles the thresholds (applied before the individual flags, explicit flag wins; the master `--agent-msgs` mode is unchanged, so `--profile heavy` alone still runs `longest`). Subagent transcripts get the SAME treatment via the shared selection path. The summed-cost == summed-emitted invariant holds with placeholders (the dropped bodies contribute zero cost; the placeholder line is charged like any emitted line).

**Ellipsis (role-asymmetric middle-truncation).** A unit over its role cap (`USER_CAP=600`, `ASST_CAP=900`, sized from measured medians) is **middle-truncated**, keeping head+tail, with an explicit `… [+K chars, L lines elided] …` marker. The assistant head is both absolutely larger (900 vs 600) and a larger fraction (0.66 vs 0.60 → head 594/tail 306 vs user 360/240), because EOT prose front-loads context and back-loads the decision. Cuts are on `char` boundaries (UTF-8 safe). No content is fabricated; nothing is silently dropped.

**WHICH PATH the per-role caps apply to.** The 600/900 caps are the **direct path's**: a plain run, a `--out` capture, and the legacy budget-driven `--slice i` used alone. **`--slices N` drops them** for a WINDOW cap (`window - SLICE_BODY_HEADROOM`), so a body renders whole unless the turn ALONE exceeds a window, and the fleet's fixed slice count is held by DISCARDING the oldest turns that do not fit rather than by shortening every turn. The two readings of one session therefore differ by design — the direct path keeps MORE turns each cut to 600/900, the fleet path keeps FEWER turns whole — and a consumer that reads a body under one path must not assume the other cap. What the direct cap costs, measured over the last 40 turns of the two largest top-level transcripts of one corpus (assistant units only, `min(full_chars, cap)` summed): on the first, 42 units / 59,965 chars → 32,404 kept at 900 (27 units cut) against 50,657 at 1,800 (12 cut); on the second, 101 units / 117,878 chars → 67,846 at 900 (48 cut) against 95,262 at 1,800 (18 cut). Raising the assistant cap to 1,800 would keep 18,253 and 27,416 more chars respectively, at roughly a 1.5x body cost on the turns it touches — recorded as a number for the owner's decision; `ASST_CAP` is unchanged.

**`L lines elided` = the newlines the CUT removed (v0.12.3).** The rendered body is the whitespace-collapsed one-line form, so it contains no newlines of its own; a unit therefore carries `newline_positions`, the char offsets in that form at which the ORIGINAL body's newlines landed (the space that replaced each newline's whitespace run; several newlines collapsed into one run share an offset and are each counted). The note counts the offsets **strictly inside the removed span** `[cut_from, cut_to)` — the head keeps `[0, cut_from)` and the tail `[cut_to, total)`, so a cut landing MID-LINE counts the newlines inside the removed span and neither of the two bounding it. Consequences, all deliberate: an **uncut** body reports 0 and prints no note; a cut that falls **between** two newlines reports 0; a **wider** cut of the same message reports **more** (the old whole-message count could not distinguish two cuts of one body, and 18 of 2,480 truncated units on two large real transcripts reported more lines than they removed characters). Positions are recorded only when the rendered text IS the normalization of the record's raw body — a **fabricated** body (an AskUserQuestion scaffold, an automation attribution label, a peer-message preview) reports no lines, because the record's own newlines sit nowhere in a string csift composed. A block-bodied message's seam between two text blocks counts as one newline (it is a line break in the record's visual form).

**The survival axis (v0.12.0) — LIVE turns only, and the cut crossed anyway.** `verbatim` replays only turns Claude Code's own conversation chain still reaches (§6.4.1). A turn the operator rewound past, or a prompt recalled and re-typed, is COUNTED in a per-session note (`abandoned  N turn(s) off the surviving conversation, not replayed: L…`, with the exact `csift show @<id> --line <n>` that fetches one) and in the JSON summary's `abandoned_turns` — never replayed. This is a correctness rule, not a display choice: the compaction summariser's input is the IN-MEMORY message array (claim CMP-020 — the driver hands the live array to a pass that slices it, and no filesystem read appears on that path), which by then no longer held that turn. Replaying it would put text into a "what the compaction clipped" reconstruction that no model ever read, which is the one thing this command must not do. The COMPLEMENT is that csift crosses a compaction CUT and Claude Code does not: the harness's loader stops its walk at a `compact_boundary` (its `parentUuid` is null), so on a resume the history above the cut has left the conversation, while csift reconstructs it anyway. The mechanism is FILE ORDER, not a link csift follows across the boundary: csift's walk stops at the same record, and the records above it are kept and flagged `pre-cut` instead of dropped, because "what the model saw at the time" is the question this command answers. Such units are flagged, never hidden: JSON `survival` reads `pre-cut` for them and `live` otherwise. Turn numbers throughout are the live ones, so a `verbatim` turn 7, a `search … --count-by turn` row `t7` and a `show --turn 7` name the same turn.
...
**Output.** Text groups under `SESSION <id>`: a budget-accounting header, then turn-by-turn `▽ Lnnnnn USER (ts)` / `[N tool calls]` / `△ Lnnnnn ASSISTANT (ts)` (one `△` line per KEPT agent message under `--agent-msgs rich`/`all`), with `══ compaction boundary · summary at Lnnnnn ══` banners at crossings, `(also in summary)` flags on demoted units, and `△ L{a}–L{b}  [X agent messages collapsed, N chars, Y tool calls, Z failed]` placeholders (plus their preview lines) for collapsed agent-message runs. `--out` writes the full (un-terminal-truncated) reconstruction to a file while the summary prints to stdout. JSON (`--format json`) emits one VERBATIM (un-truncated `text`) object per unit (`line_no`, `role`, `ts_utc`/`ts_local`, `tool_calls`, `full_chars`, `rendered_chars`, `truncated`, `elided_chars`/`elided_lines`, `also_in_summary`, `compactions_before`, `survival`) plus interleaved `{"kind":"compaction_boundary","line_no":…,"summary_chars":…}` records and, per collapsed agent-message span, a `{"kind":"collapsed_agents","agent_messages":…,"tool_calls":…,"failed":…,"first_line":…,"last_line":…,"collapsed_chars":…,"collapsed_previews":[{"line":…,"excerpt":…}]}` record. **No silent truncation** — skipped malformed lines are counted and surfaced.
...
```
(A subagent image row is suffixed `  SUBAGENT <hex> · parent <uuid>`.) **The survival axis (v0.12.0):** an image pasted into a prompt the operator later rewound past, or one recalled and re-typed, is still LISTED and still extractable — the bytes are on disk and `--out` writes the same file — it only carries a `[abandoned]` marker (`[pre-cut]` above a compaction cut) so a reader knows it is not part of the conversation the model now sees. `--turn` is the one exception: it windows on NUMBERED turns using the live numbering `search`/`show` print (§6.4.1), and an abandoned record belongs to no numbered turn, so a turn window leaves it out. The **listing/JSON** is one object per image (`handle`, `seq`, `id` [the `L<line>i<n>` locator], `line_no`, `img_index`, `session_id`, `is_subagent`, `parent_session_id`, `source_kind`, `media_type`, `b64_len`, `est_bytes`, `url`, `record_uuid`, `survival`, `ts_utc`/`ts_local`) + a trailing `{images, transcripts, skipped_lines}` summary. **Extract JSON** carries `path`, `bytes`, `media_type` (output), `source_media_type`, `converted`, and `notes` (the conversion/first-frame warnings).
...
Windows; csift's own `ps -o lstart=` call is pinned to `LC_ALL=C` and `TZ=UTC` exactly as
the harness pins its acquisition (v0.10.5: an inherited German or French locale rendered
names no English pattern parsed, and the guard was silently skipped) - every form parses
to a UTC instant, compared with a 2s tolerance; a missing/unparseable start time degrades to a pid-only probe
...
for an empty one, beside `tasks_completed`. That store is named after the id the
PROCESS started with and a `/clear` never touches it (section 4.10), so the owner's own
id is the FIRST candidate, not the only one: when it finds nothing, the ROOT of the
`cleared_from` chain is tried, then the team file whose `createdAt` is within 5 s of the
registry row's `startedAt` (its `name` is the list id; `leadSessionId` can be a throwaway
id after a `--resume` launch). A tier is read only when the tier above it found nothing,
a directory matching no candidate is NEVER read, and every store that answered prints as
`tasks store: <dir> (via own id | cleared_from root | team file)` with the same pairs in
JSON `tasks_stores` - a list recovered by inference never reads as the session's own.
A CHECKPOINT at the very tail (a `cost-state` line as the file's last) prints as one
`checkpoint` evidence row under the `tail` row it qualifies, naming the line and the four
events that write one, with JSON `last_checkpoint: {line, kind}`; there is no new verdict,
because a checkpoint is repeatable (35 of 44 over one corpus are not their file's last) and
says only that the harness stopped appending - liveness stays the registry row's answer.
With no registry row AND a tail checkpoint, that note gains the clause that the session
closed or was handed over. Human-in-the-loop blocks come from three
...
`AskUserQuestion`/`ExitPlanMode` at the main tail (measured: a multi-question ask reaches
disk while its dialog is open and a single-question ask stays buffered; a timing outcome
...
tasks_completed, tasks_stores[], last_checkpoint, pending[], background{}, last{},
tail_state, notes[]) → summary.
...
evidence[], background{}, last{}, last_checkpoint, notes[]).

### 6.15 `send` — queue a message for ONE lane, and say what will carry it (v0.11.0)

`csift send <TARGET> [MESSAGE | -f FILE | (stdin)] [--mode steer|queue] [--ttl DUR] [--from @LANE|LABEL] [--resume] [--official-only] [--format json]`

TARGET is any `@`-form csift accepts (`@<uuid>`, a uuid prefix, `@<agent-id>`, `@<Name>@<Team>`, `@main`, `@trap:<marker>`) and must resolve to EXACTLY one transcript; a routing form is translated to the transcript form for csift's own addressing and kept beside it for the official call. The body comes from the positional, `-f FILE`, or stdin.

**What it writes:** three files under `csift-channel/` (§1.1) and nothing else - `messages/<id>.json`, the receiver's `inbox/<lane>.jsonl` line, the sender's `outbox.jsonl` line.

**The caller is classified, never assumed.** Inside Claude Code (`CLAUDE_CODE_SESSION_ID` set) the caller is a LANE; the environment names only the TOP-LEVEL session in every lane, so without `--from @<lane>` the send is attributed to the session and says so on stderr. `@trap` is not attempted on this path: a trap resolution is a race csift would be running on the caller's behalf without being asked. Outside Claude Code the caller is EXTERNAL, `--from` is a free LABEL for the receipt (never a lane, never filled from the environment, never a username), and an external caller is never told to use a tool it does not have.

**The receiver is probed, then the policy table decides.** csift reads the registry row (pid liveness, status, `messagingSocketPath`), the transcript tail (running / frozen / completed / stopped-by-user / dead / unknown - `unknown` is a real answer, not a guess dressed as one), the child-lane state for a subagent target, the transcript's `version`, the receiver's merged settings (§1.2, for the configured delivery slots per event) and the runtime `armed/<lane>.json`. The table itself (`live::channel::policy::decide`) is a PURE function over those gathered facts, so every row is exercised by a unit test rather than only by a live session:

| caller | receiver | condition | channel |
|---|---|---|---|
| any | stopped-by-user lane | — | REFUSED (the harness refuses a resume for that state and tells a sender to treat the work as cancelled) |
| any | workflow lane, running | — | csift steer/queue ONLY (the official send fails closed on it) |
| any | workflow lane, completed or dead | — | REFUSED (no re-entry point: a resume by id resolves to a path the lane never wrote to, and no hook fires in a lane that has ended) |
| CC lane | top-level session, socket present, at or above the floor | — | official uds, DELEGATED, and csift queues too |
| any | top-level session, no socket | no `asyncRewake` hook on Stop | csift channel, with the risk named (nothing wakes an idle session, so the message waits for its human) |
| any | headless receiver (registry entrypoint `sdk-cli`) | — | csift channel, UNPREDICTABLE, never a promise |
| CC lane | running teammate | teams gate enabled via settings env | official mailbox, DELEGATED, and csift queues too |
| CC lane | running teammate | gate unknown | csift channel, with the gate verdict as a risk |
| CC lane | running unnamed subagent | at or above the floor | official in-process queue, DELEGATED, and csift queues too |
| subagent lane | its PARENT subagent | — | csift channel ONLY (the official `to` grammar has no parent arm: `main` reaches the top-level conversation, not the spawning agent) |
| any | completed teammate or unnamed subagent | below the floor | REFUSED (no resume path exists and no hook point will come) |
| any | completed teammate or unnamed subagent | at or above the floor, no `--resume` | REFUSED (reaching it is an official RESUME, which respawns the lane rather than delivering to it) |
| any | the same, with `--resume` | — | official resume, DELEGATED, with what a resume actually does stated |
| EXTERNAL | any | — | csift channel only (the caller holds no tool to call) |
| any | any | receiver below the official floor | csift channel only |

The official floor is Claude Code 2.1.198; an UNKNOWN version counts as below it, because the floor is a fact to be shown, not assumed. A delegated official call carries the `to` value in the routing or `a…` form, never a session uuid - the official grammar has no bare-uuid arm, so csift will not invent one and says so instead.

**ONE verdict per send**, computed after the risks: `REFUSED` (nothing queued, the reason named) · `UNPREDICTABLE` (queued, but no configured slot, or the receiver is not alive, or it is headless: csift cannot say WHEN) · `FULL` (queued, but the message renders into more chunks than the receiver has slots on any eligible event; the note says how many more would carry it in one hook round) · `MAY-FAIL` (queued, with a csift-side risk named: hooks configured but never armed, a policy switch that rewrote the hook set, a frozen lane, an unprovable gate) · `OK`. The risks that belong to the OFFICIAL leg (the mailbox's consume-deletes semantics, the in-process queue returning that a record exists rather than that anything was delivered) are reported but never lower the verdict, because the verdict is csift's promise about its OWN queue and each of those rows queues the message too. Every verdict exits 0.

**CONFIGURED is not ARMED.** `configured_slots` counts the `csift deliver --slot k` entries in the receiver's settings cascade; `armed_slots` counts slots that have actually RUN in that lane. A receiver process can predate the settings edit, so a message queued for a lane with no armed slot is queued honestly, not delivered - and that gap is a `MAY-FAIL` risk line rather than a footnote.

TEXT output: the id, verdict, channel, mode, whether it was queued, the receiver block (lane, kind, state, version, session), both gate verdicts, the configured slots per event, the armed count, the SETTINGS DISCLOSURE (the scopes that contributed, the scopes tried and absent, a `note:` line per file that was there but unreadable, and the unobservable inputs verbatim - a verdict is checkable only when its sources are named), the message size in characters and chunks, the prediction sentence (which ends with the csift leg: which event will carry part 1, and how many of the chunks fit there), the risks, and the exact official call when the send is delegated. JSON: header → one `{kind:"send", id, verdict, channel, mode, receiver{lane, routing_id, session, kind, state, version, configured_slots, armed_slots}, official{delegated, tool, to}|null, prediction, risks[], settings{sources[{scope, path, read, note}], unobservable[]}}` → `{kind:"summary", queued, chunks, message_chars, relation, cross_project, ttl_secs, official_floor_met}`.

### 6.16 `msg` / `ack` — intent joined to fact, and the receiver's own word (v0.11.0)

`csift msg [ID] [--lane @LANE] [--held|--sent|--pending] [--format json]` · `csift ack <ID> [--lane @LANE] [--format json]`

`msg <ID>` reads the message, its inbox line and its ledger lines (the INTENT), then searches the receiver lane's transcript for a `hook_additional_context` attachment whose first content string carries `id=<ID>` (the FACT). Keeping the two apart is the whole point of the command: a ledger with no matching record is a hook that printed into a process that crashed, or a write that has not flushed, and calling that a delivery would make the channel's own accounting unfalsifiable. VERDICTS (closed set): `DELIVERED` (at least one emit AND a transcript record carrying the id) · `INTENT-ONLY` (emitted, no record) · `QUEUED` (enqueued, nothing emitted) · `HELD` (a hold with no later emit; the reason is printed) · `EXPIRED` (an `expired` line, or the ttl elapsed with nothing emitted) · `ACKED` (the receiver appended an `ack` line; the fact is still reported) · `REFUSED`. The fact half on its own is exactly `csift search '<ID>' @<lane> --additional-context`, and the help says so, because a reader who knows that equivalence can audit the channel without trusting `msg`.

WITHOUT an id, `msg` lists the lane's whole ledger view, newest first, one row per message addressed to that lane, filtered by `--held` / `--sent` / `--pending`. WHICH LANE: `--lane` names it; otherwise the calling Claude Code session is used, which is the TOP-LEVEL lane in every lane, so a subagent reading its own deliveries passes `--lane @<its agent id>`. Outside Claude Code there is no caller lane and `--lane` is required.

`ack <ID>` appends `{kind:"ack"}` to the CALLING lane's ledger. It is the one statement only the receiver can make: csift can see that a chunk was emitted and that a record carrying the id exists, but only the model that read it can say it acted on it - which is also what stops a delivery hook from re-offering the message after a compaction. An EXTERNAL caller cannot ack (there is no lane to attribute the line to, so csift refuses rather than writing a record nobody owns), and an id the lane never received is a hard error naming what the lane does hold.

JSON: `msg` emits header (`{kind:"header", command:"msg", lane, session}`) → `{kind:"msg", id, verdict, mode, lane, session, emits:[{event, slot, part, parts, vehicle, ts_utc, ts_local}], held, held_reasons, expired, acked, refused_reasons, from, relation, enqueued_utc/_local, expires_utc/_local, fact:{line, uuid}|null}` rows → `{kind:"summary", messages, skipped_lines}`. `fact` is null for every verdict but `DELIVERED` and an acked delivery. `ack` emits an `{kind:"ack"}` row.

### 6.17 `deliver` — the hook entry (v0.11.0)

`csift deliver --slot <K>` · `csift deliver --recipe [--slots N] [--shell bash|powershell]`

stdin is the hook input JSON; stdout is either ONE hook-output object or nothing at all. Nobody runs this by hand: it is the line the user pastes into settings.json, once per slot, on each delivery event, and running it from a terminal just waits for a payload that never comes.

1. **Parse and refuse what cannot be placed.** The base fields (`session_id`, `transcript_path`, `cwd`, `agent_id`, `agent_type`, `hook_event_name`) plus the per-event extras `source` (SessionStart), `stop_hook_active` (Stop/SubagentStop) and `agent_transcript_path` (SubagentStop). Parsing is tolerant in one direction only: unknown fields are ignored and a field of the wrong type reads as absent, because the harness adds event extras between builds and a delivery must not die on one. But the served/remote call shape carries a `session_id` that is not a uuid and an EMPTY `transcript_path`, and treating either as a lane would write into a directory no receiver reads. Both print nothing and exit 0; where the refusal can be FILED it is, and where it cannot the exit is the whole refusal. A `session_id` that is not a uuid names no lane and no sidecar at all, so nothing is recorded; an empty `transcript_path` (or a lane id csift cannot name) under a real session uuid is recorded as a `refused` ledger line on that session's lane, because there the refusal has somewhere to live.
2. **Lane and session.** The lane is `agent_id` when the payload carries one, else `session_id`; the session is always `session_id` (the top-level uuid, in every lane). The sidecar is derived from the transcript path's own directory, so the delivery reads and writes the same tree the sender did.
3. **Refresh the armed marker** on EVERY event it handles, including the ones that deliver nothing. That marker is the only proof a SENDER can have that delivery hooks really run in this lane.
4. **Event filter.** Steer-eligible events are the eight `SessionStart`, `SubagentStart`, `PreToolUse`, `PostToolUse`, `PostToolBatch`, `UserPromptSubmit`, `Stop`, `SubagentStop`. Queue-eligible are the turn boundaries `UserPromptSubmit`, `Stop`, `SubagentStop`, plus `SessionStart` with `source` in {`resume`, `compact`} (a re-entry into an existing lane is a turn boundary for that lane even though the event name is the one that also fires at startup). Any other event exits 0 with nothing; the pasted line is identical everywhere, so the filter belongs here rather than in what the user pasted.
5. **Pending set.** A message is pending when it is not expired, not acked and has no `emit` for part 1. Two re-entry rules ride on that: `SessionStart` with `source=compact` offers an un-acked message ONCE more (a `redelivered` line marks it), because the compaction threw away the context the first delivery landed in; `source=resume` and `SubagentStart` are where a message that had no lane to land in finally lands.
6. **Slot chain, then chunks.** The pending messages render into one ordered chunk list; slot k emits chunk k, and a list shorter than k exits 0 with nothing. Slot k waits briefly for slot k-1 (§1.1) and, if it gives up, emits anyway with a one-line warning that the order may be disturbed - the message is never held hostage to the barrier. Every emit is recorded.
7. **Vehicle.** `additionalContext` at exit 0 everywhere. On `Stop`/`SubagentStop` a QUEUE message may instead go out on stderr with exit 2, which also blocks the turn from ending - but only under four conditions, each closing a different hole: the vehicle is enabled at all (`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`, default 8, a non-positive value disables it); the turn is not ALREADY continuing because a stop hook blocked it (`stop_hook_active` is the harness asking this hook to stop blocking); the lane has a block of headroom left under the ceiling; and this message has no prior exit-2 emit, because one message never chains two blocks. Otherwise the chunk rides `additionalContext` and a `held` line records `block-cap`.

**Output shape:** exactly one JSON object on stdout when there is something to deliver - `{"hookSpecificOutput":{"hookEventName":"<event>","additionalContext":"<chunk>"}}` - and nothing at all otherwise. Diagnostics never touch stdout. `--recipe` prints the settings.json `hooks` fragment for N slots (default 4) across the eight delivery events, in the plain command form or the PowerShell one (`"shell":"powershell"` on each entry), with the two-line note on stderr that the user installs it and that csift never writes a settings file; it writes nothing and exits 0. Redirecting stdout therefore yields the fragment alone.

**The envelope** a receiver sees is specified in §1.1: first chunk carries `[csift-channel v1 id=… part=1/N mode=… from=… from-session=… relation=… to=…]`, then the two sentences that place the message (it is not from your user and not from the harness; and, when the relation makes the sender a peer, that a peer has no authority over your task or your permissions), then `--- message ---`, the body chunk, `--- end ---` and a reply line. The reply line is `Reply: csift send @<sender lane> "<your reply>"` for a lane sender and, for an external one, the statement that `csift send` cannot reach it and the receiver should report in its own transcript instead - a receiver is never handed a command that would fail.

### 6.18 `whoami` lane sections, `--to` and `--peers` (v0.11.0)

The identity output of §6.3 is unchanged. After it, `whoami` prints three sections about the lane it resolved: `self` (its id, both forms for a teammate, its kind, its depth, its state and its path), `parent` (the lane above it, whether that lane is alive, and which channel a reply would take) and `topology` (the live child lanes under it as `id kind state`, then how many other live lanes exist). Under the ENVIRONMENT form the lane itself is an assumption, so its id domain is unknowable too and the JSON reports `is_subagent`/`depth` as null exactly as the identity row does; `lane_exact` and `resolved_via` say which it was.

A LANE target (`@<agent-id>`, or a teammate's routing form `@<Name>@<Team>`) answers the same three sections for THAT lane. A session uuid stays refused on this positional: `list` and `agents` inspect a different session.

`--to <TARGET>` is the reach PREDICTION: the same policy table `send` runs (§6.15), with no message written and nothing queued - it prints the channel, the verdict, both gate verdicts in the §1.2 grammar, the configured and armed delivery slots, the settings disclosure the send receipt prints (the scopes that contributed, the scopes tried and absent, a `note:` per unreadable file, and the unobservable inputs verbatim), the prediction sentence and the risks a sender would be told, and closes with the line that nothing was queued. For an AGENT target it adds what the prediction is MADE of: the harness's own four agent states are computed from in-process app state and never reach disk, so csift reads the transcript tail and probes a pid, which is an inference; the line names the carrier that remains when the inference is wrong. Only `main`, local-session and cloud targets are predictable from disk at all, and the output says so rather than implying parity.

`--peers` lists every live lane under the projects root as `id kind state` and nothing else. The omission is the specification: a description, an agent type or a name read as a role is exactly the material one lane would use to claim standing over another, so the census answers who is alive, never who should be obeyed. Live means the owning session has a registry row whose process answers a pid probe and, for a child lane, that its own tail is not settled - a child of a dead session cannot be live. A top-level lane's state is the registry's own word (`busy`/`shell`/`idle`/`waiting`); a child lane's is `in-flight`/`generating`.

OUTSIDE Claude Code (no `CLAUDE_CODE_SESSION_ID`, no target) `whoami` prints the not-a-lane answer on stdout - this process holds no lane id and no hook points, the one channel out is `csift send`, and a receiver needs `csift deliver --slot k` lines on its delivery events (printed by `csift deliver --recipe`), which its own operator pastes - and still exits non-zero, because the identity question has no answer here and §6.3's rule is that csift never guesses one.

JSON row kinds on this path: `self`, `parent` and `lane` (the sections), `reach` (`--to`), `peer` (`--peers`), each inside the standard envelope; the plain identity form keeps its `identity` row.
...
1. **Label prefilter** (cheap `memchr::memmem::Finder`, built once per needle, reused across all lines): a line that could match a requested `-t` selector must contain a marker substring — `"thinking"`, `"tool_use"`, `"tool_result"`, `"role":"assistant"`, etc. A line lacking **all** active markers is dropped pre-JSON. (Genuine-`user` is the exception: a `"type":"user"` line passing the substring gate still needs structural disambiguation from carriers, so it is parsed — but that's only ~8.8 K of 115 K lines, and only when the `user` role is active.) The marker→leaf wiring widened with the taxonomy but the mechanics are unchanged. One keep is a CONJUNCTION rather than a single needle: the D7 `compact_boundary` keep also demands the key-only `"subtype"` bytes, since the literal is a word a payload can use in prose (§4.7 carries the counting rule and the corpus numbers). The literal is tested first, so the second `memmem` runs only on the handful of lines carrying the rare word and the contract above is unaffected.
...
### 7g. The survival pass: the one cost the axis added (v0.12.0, narrowed in v0.12.1)

**The structural contract above is unchanged.** mmap, the memchr line scan, the byte
prefilter, the seek-from-EOF tail reads, the rayon fan-out and the 7f whole-file gate all
work exactly as they did, and no scan parses a payload it did not parse before. What
§4.11 added is ONE extra pass: every full-scan command now BYTE-WALKS each line its own
prefilter drops and lifts five structural fields plus the boundary fields off it
(`parse::spine_record`), because the conversation chain threads through lines no label
selector wants. The walk cannot stop early, because Claude Code writes `uuid` and `timestamp`
AFTER the payload key on 662,470 of 673,710 admitted lines (claim REC-104), so it jumps
string interiors with a SIMD `memchr2` and reads the payload bytes it never parses.
`stats` pays nothing extra for it: `parse::line_type_and_spine` lifts the same row out of
the validating parse its line-type census already runs.

**What that cost was at 0.12.0, and where it actually sat.** On a 397 MiB transcript the
pass builds 100,387 structural rows against as few as 183 records the command itself
wanted, and against 0.11.1 the 0.12.0 binary read `image` 1.74 to 1.77x,
`show --branch-points` 1.54x, `verbatim` 1.35x, `recover` 1.19x, `stats` 1.17x, a heavy
`-t user.unsent` search on a 721 MB transcript 1.17x, and `files` 0.84x (faster, because
`ChainView` replaced four per-file chain builds with one). The BYTE WALK was never that
cost: it runs inside the parallel scan, so on `image` it is 103 ms of CPU for 17 ms of
wall, and building a row is 25 ms of CPU for under 2 ms of wall. The clock time was
SERIAL. Every full-scan surface put its structural rows in the SAME vector as its own
records, so each element was sized to a `Record` at 1,184 bytes, about 120 MB was moved
twice per hand-off, and the chain build over that vector grew from 1 ms across 183 nodes
to 46 ms across 100,570.

**What v0.12.1 changed.** The two kinds are split apart from the scan down.
`parse::scan_lines_parallel_split` is the two-stream form of the parallel scan: one
`SplitVerdict` per line routes it to one of two output vectors, with the same
newline-aligned chunking, the same line numbering and the same malformed accounting, and
the single-stream scan is now that function with an uninhabited second stream, so there is
one chunking implementation rather than two. The row itself is no longer a `Record`:
`parse::SpineRow` carries the physical line, a `SpineKind` tag instead of a decoded type
string, `uuid`, `parentUuid`, the RAW `timestamp` and `isSidechain`, with the rare fields
behind one `Option<Box<SpineExtra>>` an ordinary attachment row never allocates.
`size_of::<SpineRow>()` is 72 bytes against `size_of::<Record>()` 1,184, and the width is
asserted by a unit pin rather than assumed. Consumers merge the two streams back by line
into a BORROWED file-order row list (`search::Row`, plus a local merge of the same shape
in `stats` and in `show --branch-points`), which is the index space every chain answer is
addressed by, and the chain reads both kinds through ONE shape, `model::ChainNode`, whose
message-derived accessors answer the same `false`/`None` a message-less record gave. The
timestamp stays a raw string because the chain runs it through the same `timez::epoch_ms`
at the same two call sites, so neither kind can answer a timestamp question the other
would answer differently. `SummarizeIndex::from_nodes` runs the C-33 pairing over the same
merged list, because a compaction boundary is a spine row under every selector that does
not reach the boundary leaf.

**The measurement.** Medians of 21 reps per cell, four arms (0.11.1, 0.12.0, this tip and
a same-binary control), the arm order rotated by rep index and reversed on odd reps, wall
from a monotonic clock and CPU from the child's own rusage. Six shapes run on the 397 MiB
transcript and the heavy `-t user.unsent` search on the 721 MB one. Against 0.12.0:
`image` -29.2%, `verbatim` -14.6%, `show --branch-points` -14.2%, `files` -8.1%, the heavy
search -6.8%, `stats` -6.7%, `recover` -6.1%. Every shape is faster and none is slower,
and the same-binary control band is at most 1.1% over the seven, so the smallest win is
five times the drift. CPU medians move the same way (`image` 0.808x, `verbatim` 0.898x,
`show --branch-points` 0.889x of 0.12.0). A second, independent run of the same driver
read the same direction with larger margins (`image` -31.8%, `verbatim` -16.0%). The two
runs are reported side by side rather than averaged. The 0.12.0 text put a lower bound of
13% on `image` and 8% on `verbatim` for this patch, and both are beaten.

**What is left, and why it stays.** Against 0.11.1: `image` 1.233x, `show --branch-points`
1.300x, `verbatim` 1.153x, `recover` 1.125x, the heavy search 1.089x, `stats` 1.073x, and
`files` 0.776x. That residue is the axis itself, the parallel byte walk plus one chain
build over the whole DAG, and both are what the answer is made of. An early stop in the
key walk is impossible (REC-104, above), and a reverse scan from the line end lands within
one percent of the forward walk on every shape.
...
2. Kind-tagged rows — list→`session`, search→`exchange` | `census`, show→`record`,
   ...
   compaction_boundary|collapsed_agents`, plan→`plan|binding|plan-edit|plan-unbound-text`,
   ...
   send→`send`, msg→`msg`, ack→`ack`, and whoami→`identity` plus the v0.11.0 lane rows
   `self` | `parent` | `lane` (the three sections), `reach` (`--to`) and `peer`
   (`--peers`). `deliver` is the one command with NO envelope: its stdout is the
   harness's own hook-output object, or nothing (§6.17).
3. `{"kind":"summary", …}` — the LAST line, always (even all-zero counts). Notable
   ...
   `{records, dropped_by_cap, refetch_remainder, non_record_lines, skipped_lines,
   with_elicitation_sidecar}`; agents → `{sessions, runs, agents}`; send →
   `{queued, chunks, message_chars, relation, cross_project, ttl_secs,
   official_floor_met}`; msg → `{messages, skipped_lines}`; whoami's reach form →
   `{queued:false, refused}` (the prediction wrote nothing, stated as data).
...
**The v0.12.0 additions, all of them beside `labels[]` and never inside it.** The survival
verdict is a fact ON a row, so it rides its own keys and no leaf name encodes it (§5.6):
`survival` (`live` | `pre-cut` | `abandoned`) plus `abandoned_root_line` and
`replay_copy_of` on every `search` hit, every `show` record, every `files` mutation and
boundary row, every `verbatim` turn unit and every `image` listing row; an abandoned row
carries a NULL `turn_index` beside them. The `search` summary adds `abandoned_records`,
`rewound_turns`, `replay_copies`, `boundary_cut_line` and `leaf_source` (`last-prompt` |
`tail` | `explicit`, which gate chose the leaf); the `verbatim` summary adds
`abandoned_turns`; a `recover` coverage or snapshot row adds `abandoned_events`; `files`
grouped rows add `abandoned` as a SHARE of `total`, never a subtraction; `stats` session
rows and the scope TOTAL add the whole-file chain totals `abandoned_turns`,
`rewound_turns` and `replay_copies`, which never window. Beside the axis: `resume_paired`
on a `harness.resume.placeholder` hit (null on every other row), `mode` (`compact` |
`summarize-from-here` | `summarize-up-to-here`, null when a boundary pairs with no
summary) and the verbatim `compact_metadata` object on both compaction records, `line`,
