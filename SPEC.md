...
folds closed ones to counts (completed / failed / killed / stopped / timed out);
launches come from every lane, completions from the main file only, joined by the
...
output: a skipped reuse guard, a missing registry row, and the honesty note that a
pending PERMISSION prompt lives only in CC process memory and would masquerade as
idle. Growth alone is never activity (a main transcript grows while idle - enqueues,
