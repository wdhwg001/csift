...
**The blind region, and the FLOOR that protects it.** csift steps OVER a compaction cut through the boundary's own `logicalParentUuid`, which Claude Code's walk never reads, and that step can fail: 52 of 245 `compact_boundary` records, across 14 of 75 transcripts, name a uuid that is on NO line of their own file, the pre-compaction records having been deleted from disk by the pre-boundary deletion path, and on 13 of those files that dangling boundary is where the walk ends (claim MISC-043). Everything physically above such a point is a region csift cannot resolve. So the walk's terminating record is a FLOOR: an off-chain record above it reads `PreCut` and is NEVER called abandoned, and inside that region the measured same-parent opener rule still marks a draft, so draft detection never depends on the chain reaching that far. This is csift's own failure mode and not the loader's, and it is NOT a partial record set: no admitted record's `parentUuid` names a progress or unparsed line (12 transcripts probed, empty census). The step is worth taking anyway: over the 21 compacted transcripts, conversation-record coverage is a median 9.4% without it and 46.5% with it.
...
> ③ **TWO fail-open guards, each with its own measured cause.** csift's §7d prefilter keeps only role-bearing lines, and a chain built from those alone resolves nothing: 1 of 30,613 conversation records reachable on one real session. So every surface splices back the STRUCTURAL row of each line its own prefilter dropped, through `parse::spine_record` - a depth-1 key walk lifting `type`, `uuid`, `parentUuid`, `timestamp`, `isSidechain` and the boundary fields into an otherwise EMPTY record, never the payload. The SECOND guard is a FLOOR, and its cause is csift's own forensic step, not a gap in the record set. csift STEPS OVER a compaction cut through the boundary's `logicalParentUuid` and flags what it then reads `PreCut`: Claude Code drops that history, csift keeps reading it, every selector still reaches it, because what the model saw AT THE TIME is the question an archive answers. Claude Code's walk never takes that step and so never has to care that the target may be GONE. MISC-043: 52 of 245 `compact_boundary` records, across 14 of 75 transcripts, name a `logicalParentUuid` matching no `uuid` on any line of their own file - the pre-compaction records were deleted from disk by the pre-boundary deletion path - and on 13 of those files that dangling boundary is where the walk ends. Everything physically above such a point is a BLIND REGION csift cannot resolve, so the walk's terminating record floors the verdict: an off-chain record above it is `PreCut`, NEVER `Abandoned`, and inside that region the measured same-parent opener rule still marks a draft, so draft detection never depends on the chain reaching that far. This is csift's own failure mode and not the loader's, and it is not a partial record set: no admitted record's `parentUuid` names a progress or unparsed line (12 transcripts probed, empty census). The step earns its keep - over the 21 compacted transcripts, conversation-record coverage is a median 9.4% without it and 46.5% with it.
