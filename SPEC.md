...
TEXT output: the id, verdict, channel, mode, whether it was queued, the receiver block (lane, kind, state, version, session), both gate verdicts, the configured slots per event, the armed count, the message size in characters and chunks, the prediction sentence (which ends with the csift leg: which event will carry part 1, and how many of the chunks fit there), the risks, and the exact official call when the send is delegated. JSON: header → one `{kind:"send", id, verdict, channel, mode, receiver{lane, routing_id, session, kind, state, version, configured_slots, armed_slots}, official{delegated, tool, to}|null, prediction, risks[]}` → `{kind:"summary", queued, chunks, message_chars, relation, cross_project, ttl_secs, official_floor_met}`.
