...
### 7g. The survival pass: the one cost the axis added (v0.12.0)
...
**The measured price, on the heaviest shapes.** On a 397 MB transcript the pass builds
100,387 structural rows against as few as 183 records the command itself wanted. Against
0.11.1 on that transcript, medians of 21 runs per arm on an interleaved CPU-time driver with
rotated cells and a same-binary control drifting under 2%, the ratios quoted being wall
clock: `image` 1.74 to 1.77x,
`verbatim` 1.35x, `show --branch-points` 1.54x, `recover` 1.19x, `stats` 1.17x, and a
heavy `-t user.unsent` search on a 721 MB transcript 1.17x. `files` reads 0.84x, faster,
because `ChainView` replaced four per-file chain builds with one. `image` pays the most
because it parsed almost nothing before.

**Where the time actually goes, and the four levers that did not work.** The byte walk
runs INSIDE the parallel scan, so on `image` it is 103 ms of CPU and 17 ms of wall, and
building the row is 25 ms of CPU and under 2 ms of wall. The clock time is serial: two
bulk moves of a row vector at 1,184 bytes a row, plus the chain build itself, which grows
from 1 ms over 183 nodes to 46 ms over 100,570. An early stop in the key walk is
impossible (REC-104, above). A reverse scan from the line end lands within one percent of
the forward walk on every shape. A leaner row buys about 1.5% on `search` and half a
percent on `image` and `verbatim`, because its construction was never the cost. The
remaining headroom is the serial carry, which needs the structural rows to stop sharing a
collection with full records: measured at a lower bound of 13% on `image` and 8% on
`verbatim`, and scheduled as a later patch.
