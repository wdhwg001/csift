...
```
The `s<k>·t<n>` header carries the turn index (`t270` => `--turn-range 270..270`). JSON per Exchange (leading `session_header` if `S>0`; row trio): `{…trio…, turn_index, ts_utc, ts_local, hits:[{category, line, uuid, ts_utc, ts_local, tool_name, excerpt, image_ids:[…]}], record_uuids:[…]}` (+`siblings:[…]` same shape under `--siblings`). `image_ids` (`#N`/`L<line>i<n>`) feed `image --id` (see §image). CLOSED UNTAGGED `{matched, sessions, dropped_by_cap, skipped_lines, unresolved:[…], with_elicitation_sidecar, excerpts_truncated}` (`unresolved` = explicit `--line`/`--uuid` tokens that hit nothing; `excerpts_truncated` = ≥1 emitted excerpt was clipped → re-fetch full via `--no-truncate`/`--line`/`--uuid`). `-c` JSON = `{"matched":N}` (header-free).
...
- **`search`/`turns`/`list`** include the unresolved-pending records as if native - all three elicitation kinds surface under `-t tool` (an AUQ/ExitPlanMode via its `tool_use`; the MCP `system` record via its `content`, so `csift search <elicitation-text>` finds even an MCP elicit); turns shows each as the latest turn unit; list annotates the row. Once the elicitation is ANSWERED, CC wrote the real record and the pending is paired off (dropped) → no duplicates, automatic.
