...
| what I typed but esc-recalled is gone | a sent-then-esc-recalled draft IS on disk — `-t user.unsent` finds it (7 in one real session, one a 2.48M-char paste). What is genuinely gone from user RECORDS: a QUEUED message edited before dispatch — but its bytes are on disk in a `queue-operation` line, searchable since v0.9.5 as `-t user.queued` (measured over 6 sessions: 19-28% of the human's enqueued texts never became a user record; an earlier 61% figure counted every queue operation, remove lines included, over 3 sessions) |
...
- status `verdict`: `{verdict, evidence:[{surface (registry|pid|tail|children|sidecar), value, age_secs}], children:[{session_id, state (in-flight|generating|settled), detail} — live lanes only], settled_children, tasks:[{id, subject, status, blocked_by}] (null = no tasks dir; [] = a dir with nothing), tasks_completed, pending:[…], notes:[…]}`; summary `{verdict}`. wait exit object: `{kind:"wait", fired (condition|"timeout"), verdict, waited_secs, evidence:[…]}`.
