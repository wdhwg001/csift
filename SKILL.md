...
| "re-run the @trap command" = a second line in my script | the retry must be a NEW, SEPARATE Bash tool invocation — two attempts inside ONE shell script are still ONE in-flight command (nothing flushes until the whole script exits), so both miss; batching the retry to save a round trip is exactly the wrong move here |
