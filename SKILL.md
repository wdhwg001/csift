...
description: >-
  ...
  documented traps that return plausible wrong answers with no error (a user-role
  filter overcounts human turns 3.03x; 16.5% of human turns hide inside tool_result
  payloads; an AUQ answer can read (notes only) with the real words in annotations) -
  reach for it the moment you would hand-scan or hand-parse a session jsonl, or would
  shadow session facts in your own state file. Find where a phrase or regex appears
  across ALL sessions, with timestamps and lines; read records by line, turn or uuid;
  see what a session is doing now; see tools, tokens, models, files changed; extract
  pasted images and screenshots back to files you can read, so an image-bearing turn
  is evidence, not a blank; recover a file or plan even when deleted; restore the
  verbatim turns a compaction summary clipped. Read-only, sub-second, safe inside
  hooks. Pure regex, not semantic search.
---

# csift — ripgrep for Claude Code session transcripts

Surface: **v0.8.1** (must == `csift --version`). If an invocation you were CONFIDENT about errors, your knowledge is stale — an older csift surface from prefill/summary/habit. Re-read THIS file (it always matches the installed binary); never fall back to hand-parsing the jsonl.
...
## Labels (`-t/--label` · `-T/--label-not`) — dotted `role.class.sub`, 3 roles, 26 leaves
...
- Superseded drafts (esc-edit resends: an opener replaced by a later same-parent sibling) are collapsed by turn reconstruction — correct hygiene, and DISCLOSED: the footer prints `(N superseded draft(s) collapsed …)` and the JSON summary carries `superseded_drafts`. A draft is still a real record: fetch it with `show --line/--uuid` (renders annotated, outside turn numbering).
...
```
Reads `$CLAUDE_CODE_SESSION_ID` (alias `$CODEX_COMPANION_SESSION_ID`). Neither set ⇒ errors with guidance — never guesses by mtime. LANE HONESTY (v0.8.2): the env names the TOP-LEVEL session in EVERY lane, so the env form reports `is_subagent`/`parent_session_id`/`depth` as **null** (unknowable, never a fabricated false/0), prints a `lane unknown from env alone` text line, and notes the resolution path on stderr; every `@main` resolution likewise prints an unconditional stderr lane note. A first-try `@trap` HIT is lane PROOF; a miss proves nothing. Subagent caveat: current CC hands an Agent-tool subagent the PARENT session's id (the subagent's OWN id is withheld from env; workflow `agent()` likewise; older builds handed a Task subagent its own id) — so from a subagent, plain `whoami` usually resolves the ROOT; `@trap` resolves the true self env-independently and returns the full upstream chain (self depth 0 → root). @trap timing: subagent = first try; MAIN = first use always misses (use `@main`, or re-run the SAME command+marker) — see §trap.
...
- search `exchange`: trio + `turn_index, ts_utc/local, record_uuids, hits[]`; each hit: trio (v0.6.5 — bare `.hits[]` flattening keeps real ids) + `label, labels[], line (null=sidecar), uuid, ts_utc/local, tool_name, from, to, pairing, is_error (result hits: true|false; else null), tool_use_id, source, excerpt, image_ids[], refetch`. Summary: `matched, sessions, transcript_ids[≤100], transcript_ids_truncated, dropped_by_cap, skipped_lines, superseded_drafts, with_elicitation_sidecar, excerpts_truncated` (+ on zero matches `definitive_absence, active_filters, excluded_by_label`).
...
- stats `session`: trio + `lines, line_types{type→count} (whole-file, never windowed), user_records, assistant_records, turns, compactions, first_utc/local, last_utc/local, tokens{model→{input,output,cache_read,cache_creation}}, tools{name→count}, skipped_lines`. Summary adds scope totals (`tokens`, `tools`, `turns`, `line_types`).
