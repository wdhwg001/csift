...
- **EXCERPT TRUNCATION (defeat it!)**: each hit shows a **~400-char excerpt CENTERED on the match**, elided `… (+N chars)`. **`--full`/`--no-truncate`** = whole record every hit; `--line`/`--uuid`-addressed records always render FULL. **An excerpt is a match-centered FRAGMENT, NOT a summary** — its opening can read OPPOSITE to the record's full intent, so don't conclude from a clipped fragment alone. When ≥1 excerpt was actually clipped, text output prints a TRAILING caution (re-run `--full`, or fetch one record via `--line <N>`/`--uuid <U>`) and JSON sets the summary's `excerpts_truncated:true`; both auto-suppress under `--full`/`--line`/`--uuid` (cap lifted ⇒ nothing clipped).
