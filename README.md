...
|---|---:|---:|---|
| end-to-end | 532 | 81.3% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced | 532 | 81.3% | the three hops quoted at byte offsets in `producer_chain`; the anchors line below is the measured state of those offsets |
...
| specimen-only | 28 | 4.3% | observed on disk or live; the writer not traced (or traced only in part) (24 of them with a partly traced writer) |
| producer-only | 41 | 6.3% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 39 | 6.0% | a template or field located without its gate and trigger; no specimen |
...
| total | 654 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |

A traced claim pins each hop at a byte offset in that build, and the gate re-reads every one of them:

`anchors byte-exact at Claude Code 2.1.258: 6,400 of 7,009 (elided 343, absent 183, prefix-only 83)`
