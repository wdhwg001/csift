...
|---|---:|---:|---|
| end-to-end | 496 | 79.4% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 496 | 79.4% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| producer-only | 32 | 5.1% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 47 | 7.5% | a template or field located without its gate and trigger; no specimen |
| by-elimination | 0 | 0.0% | neither leg; attributed by exclusion or from csift's own design |
| upstream | 14 | 2.2% | the producer lies outside the shipped binary by construction (the model or API side, the operating system, a native runtime binding); the client-side treatment is traced and a specimen observed |
| total | 625 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
