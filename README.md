...
|---|---:|---:|---|
| end-to-end | 488 | 89.7% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 488 | 89.7% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| specimen-only | 5 | 0.9% | observed on disk or live; the writer not traced (or traced only in part) (3 of them with a partly traced writer) |
| producer-only | 30 | 5.5% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 7 | 1.3% | a template or field located without its gate and trigger; no specimen |
...
| total | 544 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
