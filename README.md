...
|---|---:|---:|---|
| end-to-end | 488 | 83.4% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 488 | 83.4% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| specimen-only | 26 | 4.4% | observed on disk or live; the writer not traced (or traced only in part) (15 of them with a partly traced writer) |
| producer-only | 32 | 5.5% | the writer traced in full; no specimen exists in the corpus or could be produced here |
...
| total | 585 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
