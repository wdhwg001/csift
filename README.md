...
|---|---:|---:|---|
| end-to-end | 488 | 80.5% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 488 | 80.5% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| specimen-only | 33 | 5.4% | observed on disk or live; the writer not traced (or traced only in part) (21 of them with a partly traced writer) |
| producer-only | 32 | 5.3% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 39 | 6.4% | a template or field located without its gate and trigger; no specimen |
...
| total | 606 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
