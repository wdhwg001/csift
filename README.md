<div align="center">
  ...
  <p>
    ...
    <img alt="coverage: 96.4%" src="https://img.shields.io/badge/coverage-96.4%25-4ade80" />
    <img alt="mutation score: 91.6%" src="https://img.shields.io/badge/mutation%20score-91.6%25-a3e635" />
...
|---|---:|---:|---|
| end-to-end | 539 | 81.4% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced | 539 | 81.4% | the three hops quoted at byte offsets in `producer_chain`; the anchors line below is the measured state of those offsets |
...
| specimen-only | 29 | 4.4% | observed on disk or live; the writer not traced (or traced only in part) (24 of them with a partly traced writer) |
| producer-only | 41 | 6.2% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 39 | 5.9% | a template or field located without its gate and trigger; no specimen |
...
| total | 662 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |

A traced claim pins each hop at a byte offset in that build, and the gate re-reads every one of them:

`anchors byte-exact at Claude Code 2.1.258: 6,446 of 7,055 (elided 343, absent 183, prefix-only 83)`
