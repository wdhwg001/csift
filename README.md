...
|---|---:|---:|---|
| end-to-end | 559 | 83.3% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced | 559 | 83.3% | the three hops quoted at byte offsets in `producer_chain`; the anchors line below is the measured state of those offsets |
...
| partial-producer | 11 | 1.6% | a template or field located without its gate and trigger; no specimen |
...
| total | 671 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |

A traced claim pins each hop at a byte offset in that build, and the gate re-reads every one of them:

`anchors byte-exact at Claude Code 2.1.258: 6,726 of 7,333 (elided 342, absent 183, prefix-only 82)`
