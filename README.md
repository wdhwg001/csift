...
|---|---:|---:|---|
| end-to-end | 488 | 84.9% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 488 | 84.9% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| specimen-only | 16 | 2.8% | observed on disk or live; the writer not traced (or traced only in part) (12 of them with a partly traced writer) |
| producer-only | 32 | 5.6% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 25 | 4.3% | a template or field located without its gate and trigger; no specimen |
| by-elimination | 0 | 0.0% | neither leg; attributed by exclusion or from csift's own design |
| upstream | 14 | 2.4% | the producer lies outside the shipped binary by construction (the model or API side, the operating system, a native runtime binding); the client-side treatment is traced and a specimen observed |
| total | 575 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
