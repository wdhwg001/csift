...
|---|---:|---:|---|
| end-to-end | 488 | 90.2% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced and adversarially re-read | 488 | 90.2% | the three hops quoted at byte offsets in `producer_chain`, every excerpt re-read by an independent verifier |
...
| specimen-only | 2 | 0.4% | observed on disk or live; the writer not traced (or traced only in part) (2 of them with a partly traced writer) |
...
| total | 541 | 100.0% | one claim per Claude Code behavior csift depends on, verified at Claude Code 2.1.258 |
<!-- ledger-tally:end -->
