...
|---|---:|---:|---|
| end-to-end | 555 | 83.8% | the writer, its gate and its trigger read in the shipped binary, and a specimen observed on disk or live |
| &nbsp;&nbsp;of which chain traced | 555 | 83.8% | the three hops quoted at byte offsets in `producer_chain`; the anchors line below is the measured state of those offsets |
...
| specimen-only | 13 | 2.0% | observed on disk or live; the writer not traced (or traced only in part) (8 of them with a partly traced writer) |
| producer-only | 70 | 10.6% | the writer traced in full; no specimen exists in the corpus or could be produced here |
| partial-producer | 10 | 1.5% | a template or field located without its gate and trigger; no specimen |
...
`anchors byte-exact at Claude Code 2.1.258: 6,680 of 7,287 (elided 342, absent 183, prefix-only 82)`
