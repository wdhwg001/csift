...
- **Subcommands:** `list`, `search`, `agents`, `whoami`, `files`, `recover`, `plan`, `turns`. `list`/`search`/`files`/`recover`/`turns` span each session's subagent transcripts by default (`--no-subagents` opts out); `agents` reports a session's subagent lifecycle (kind / start / completion / status), with `--since`/`--until` + `--by start|completion` window filters. `recover --file @plan` reconstructs the session-bound plan file; `plan` locates it (via the `plan_mode` attachment). See SPEC §6.5–6.7.
